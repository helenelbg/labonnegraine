...
{if $products}
					...
					<tr id="product_{$product.id_product}_{$product.id_product_attribute}_{if $quantityDisplayed > 0}nocustom{else}0{/if}_{$product.id_address_delivery|intval}{if !empty($product.gift)}_gift{/if}" class="cart_item{if isset($productLast) && $productLast && (!isset($ignoreProductLast) || !$ignoreProductLast)} last_item{/if}{if isset($productFirst) && $productFirst} first_item{/if}{if isset($customizedDatas.$productId.$productAttributeId) AND $quantityDisplayed == 0} alternate_item{/if} address_{$product.id_address_delivery|intval} {if $odd}odd{else}even{/if}">
						...
						{if !isset($noDeleteButton) || !$noDeleteButton}
							...
							{if (!isset($customizedDatas.$productId.$productAttributeId) OR $quantityDisplayed > 0) && empty($product.gift)}
								<div>
									<a rel="nofollow" title="{l s='Delete'}" class="cart_quantity_delete" id="{$product.id_product}_{$product.id_product_attribute}_{if $quantityDisplayed > 0}nocustom{else}0{/if}_{$product.id_address_delivery|intval}" href="javascript:;"  onclick="WishlistProductManage('wlp_bought', 'delete', '{$id_wishlist}', '{$product.id_product}', '{$product.id_product_attribute}', $('#quantity_{$product.id_product}_{$product.id_product_attribute}').val(), $('#priority_{$product.id_product}_{$product.id_product_attribute}').val()); $(this).closest('tr').remove();"><i class="icon-trash"></i></a>
								</div>
							{else}
						...
						<td class="cart_total" data-title="{l s='Panier'}">
							...
							{if $product.attribute_quantity <= 0}
								...
								<a class="button btn btn-default me_prevenir" data-id-product="{$product.id_product}" href="#" title="{l s='Add to cart'}" onclick="addNotification({$product.id_product},{$product.id_product_attribute}); return false;" >
    ...
    {if !$refresh}
        ...
        {if count($productsBoughts)}
			...
			<button type="button" class="addAll" name="button">
				Tout ajouter au panier
			</button>
			<br>
			<button type="button" class="bouton_partage" onclick="shareWishListModal('{$wishlist_url_token}{$token_wish}');">
				<img src="/themes/default-bootstrap/img/picto_partager.png" alt=""> Partager ma liste à un ami
			</button>
        {/if}
    {/if}
{else}
