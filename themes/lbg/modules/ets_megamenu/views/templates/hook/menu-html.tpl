...
{if isset($category)}
	...
	{if isset($category->id_parent)}
      {$categoryParentId = $category->id_parent} {* Pages fiches produit *}
    {elseif isset($category.id_parent)}
      {$categoryParentId = $category.id_parent} {* Pages listing produit *}
    {/if}
{/if}
...
{if isset($menus) && $menus}
    ...
    <ul class="mm_menus_ul {if isset($mm_config.ETS_MM_CLICK_TEXT_SHOW_SUB) && $mm_config.ETS_MM_CLICK_TEXT_SHOW_SUB} clicktext_show_submenu{/if} {if isset($mm_config.ETS_MM_SHOW_ICON_VERTICAL)&& !$mm_config.ETS_MM_SHOW_ICON_VERTICAL} hide_icon_vertical{/if}">
        {foreach from=$menus item='menu'}
			{$active = ''}
			{if $menu.id_category == $categoryId || $menu.id_category == $categoryParentId}
				{$active = 'active '}
			{/if}
            ...
            <li data-id-category="{$menu.id_category}" class="{$active}mm_menus_li{if $menu.enabled_vertical} mm_menus_li_tab{if $menu.menu_ver_hidden_border} mm_no_border{/if}
