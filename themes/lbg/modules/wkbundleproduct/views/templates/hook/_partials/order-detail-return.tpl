...
{block name='order_products_table'}
  <form id="order-return-form" action="{$urls.pages.order_follow}" method="post">
      ...
      <table id="order-products" class="table table-bordered return">
        <thead class="thead-default">
          <tr>
            ...
            <th>{l s='Product' mod='wkbundleproduct'}</th>
            <th>{l s='Quantity' mod='wkbundleproduct'}</th>
            <th>{l s='Returned' mod='wkbundleproduct'}</th>
            <th>{l s='Unit price' mod='wkbundleproduct'}</th>
            <th>{l s='Total price' mod='wkbundleproduct'}</th>
          </tr>
        </thead>
        {foreach from=$order.products item=product name=products}
          <tr>
            <td>
              ...
              {/if}
               <br>
                {if $product_array && $product_array[$product.id_order_detail]}
                  {l s='This bundle contains following products - ' mod='wkbundleproduct'}
                  <ol>
                    {foreach $product_array[$product.id_order_detail] as $info}
                      {if isset($info.download_link) && $info.download_link}
                      <li><a href="{$info.download_link}">{$info.product_name}</a>&nbsp;|&nbsp;{l s='Quantity' mod='wkbundleproduct'} :{$info.product_qty}</li>
                      {else}
                      <li>{$info.product_name}&nbsp;|&nbsp;{l s='Quantity' mod='wkbundleproduct'} :{$info.product_qty}</li>
                      {/if}
                    {/foreach}
                  </ol>
                {/if}
            ...
            <td>
              ...
              {if $product.reference}
                {l s='Reference' mod='wkbundleproduct'}: {$product.reference}<br/>
              {/if}
              {if $product.customizations}
                {foreach from=$product.customizations item="customization"}
                  <div class="customization">
                    <a href="#" data-toggle="modal" data-target="#product-customizations-modal-{$customization.id_customization}">{l s='Product customization' mod='wkbundleproduct'}</a>
                  </div>
                      ...
                      <div class="modal-dialog" role="document">
                        <div class="modal-content">
                          <div class="modal-header">
                            ...
                            <h4 class="modal-title">{l s='Product customization' mod='wkbundleproduct'}</h4>
                          </div>
            ...
            <div class="content">
              <div class="row">
                <div class="col-sm-5 desc">
                  ...
                  {if $product.reference}
                    <div class="ref">{l s='Reference' mod='wkbundleproduct'}: {$product.reference}</div>
                  {/if}
                  {if $product.customizations}
                    {foreach $product.customizations as $customization}
                      <div class="customization">
                ...
                <div class="col-sm-7 qty">
                  <div class="row">
                    ...
                    <div class="col-xs-4">
                      {if $product.customizations}
                        {foreach $product.customizations as $customization}
                          <div class="q">{l s='Quantity' mod='wkbundleproduct'}: {$customization.quantity}</div>
                          <div class="s" id="_mobile_return_qty_{$product.id_order_detail}_{$customization.id_customization}"></div>
                        {/foreach}
                      {else}
                        <div class="q">{l s='Quantity' mod='wkbundleproduct'}: {$product.quantity}</div>
                        {if $product.quantity > $product.qty_returned}
                          <div class="s" id="_mobile_return_qty_{$product.id_order_detail}"></div>
                        {/if}
                      {/if}
                      {if $product.qty_returned > 0}
                        <div>{l s='Returned' mod='wkbundleproduct'}: {$product.qty_returned}</div>
                      {/if}
                    </div>
    ...
    <div class="box">
      <header>
        <h3>{l s='Merchandise return' mod='wkbundleproduct'}</h3>
        <p>{l s='If you wish to return one or more products, please mark the corresponding boxes and provide an explanation for the return. When complete, click the button below.' mod='wkbundleproduct'}</p>
      </header>
      ...
      <footer class="form-footer">
        ...
        <button class="form-control-submit btn btn-primary" type="submit" name="submitReturnMerchandise">
          {l s='Request a return' mod='wkbundleproduct'}
        </button>
      </footer>
    </div>
