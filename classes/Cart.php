    ...
    public function isAllProductsInStock($ignoreVirtual = false, $exclusive = false)
    {
        ...
        foreach ($this->getProducts(false, false, null, false) as $product) {
            ...
            if (!$exclusive
                ...
                if ( $_SERVER['REMOTE_ADDR'] == '80.15.118.113' || $_SERVER['REMOTE_ADDR'] == '176.134.110.234' )
                {
                    error_log('P '.$product['id_product'].' : productQuantity : '.$productQuantity.' / availableOutOfStock : '.$availableOutOfStock);
                }
                return false;
            } elseif ($exclusive) {
                if ($productQuantity <= 0) {
                ...
                if ($productInStock > 0 && $productOutOfStock > 0) {
                    if ( $_SERVER['REMOTE_ADDR'] == '80.15.118.113' || $_SERVER['REMOTE_ADDR'] == '176.134.110.234' )
                {
                    error_log('P '.$product['id_product'].' : productInStock : '.$productInStock.' / productOutOfStock : '.$productOutOfStock);
                }
                    return false;
                }
            }
