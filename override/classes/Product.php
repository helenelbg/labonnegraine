...
class Product extends ProductCore {
	...
	public static function getCategoriesPlantsCorresp($categories = []) {
		$plant_categories = [21=>349,62=>353,69=>354,71=>355,77=>356,195=>350,45=>351,46=>352];
        foreach($categories as $cat)
        {
            if ( isset($plant_categories[$cat]) && !empty($plant_categories[$cat]) )
            {
                return $plant_categories[$cat];
            }
        }
		return false;
	...
	public static function equivalentEnplant($categories = []) {
		$plant_categories = Product::getCategoriesPlants();
		if(!is_array($categories)){
			$categories = [$categories];
		}

		return (!empty(array_intersect($plant_categories, $categories)));
	}
