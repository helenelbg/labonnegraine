...
die;
  ...
  function dupliquer_commande($id_commande)
  {
    ...
    if ( !in_array($id_commande, $histo) )
    {
      ...
      $etat_commande=39; //3 preparation en cours
	...
	$history->changeIdOrderState($etat_commande, $new_order->id);
      ...
      $req_dup_reg='SELECT DISTINCT od.id_order FROM `'.
              ...
              . ' WHERE o.id_customer = "'.$old_order->id_customer.'" AND o.id_order <> "'.$id_commande.'" AND o.id_order <> "'.$new_order->id.'" AND od.product_name LIKE "%Conditionnement : plant%" AND o.current_state NOT IN (1,6,7,8,10) AND od.id_order >= "220261" AND o.payment <> "X";';
      $T_lignes_dup_reg = Db::getInstance()->executeS($req_dup_reg);
      ...
      if ( !empty($testcoli['id_order']) )
      {
        ...
        $req_testcolir='SELECT * FROM `'._DB_PREFIX_.'colissimo_cart_pickup_point` WHERE id_cart = "'.$old_order->id_cart.'";';
        $testcoliRangeer = Db::getInstance()->executeS($req_testcolir);
        $testcolir = $testcoliRangeer[0];
        if ( !empty($testcolir['id_cart']) )
        {
          $req_colissimor="INSERT INTO `ps_colissimo_cart_pickup_point` (id_cart, id_colissimo_pickup_point, mobile_phone)
          VALUE ('".$new_order->id_cart."', '".$testcolir['id_colissimo_pickup_point']."', '".$testcolir['mobile_phone']."')";
          Db::getInstance()->executeS($req_colissimor);
        }
      }
  ...
  $req_dup='SELECT DISTINCT od.id_order, o.id_customer FROM `ps_order_detail` od JOIN ps_orders o on (od.id_order=o.id_order) WHERE od.product_name LIKE "%Conditionnement : plant%" AND o.current_state NOT IN (1,6,7,8,10) AND od.id_order >= "220261" AND o.payment <> "X";';
  echo $req_dup.'<br />';
