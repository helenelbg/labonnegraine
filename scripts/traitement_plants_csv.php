<?php
  ...
  $req_dup='SELECT DISTINCT od.id_order FROM `ps_order_detail` od JOIN ps_orders o on (od.id_order=o.id_order) WHERE od.product_name LIKE "%Conditionnement : plant%" AND o.current_state IN (39) AND od.id_order >= "223507";';
  ...
  foreach($T_lignes_dup as $T_ligne_commande)
  {
    ...
    if ( !in_array($id_commande, $cmd_ignore) )
    {
        ...
        $keyEC = $id_commande;
        //$req_dup='SELECT DISTINCT od.id_order FROM `ps_order_detail` od JOIN ps_orders o on (od.id_order=o.id_order) WHERE o.id_order <> "'.$id_commande.'" AND o.id_customer = "'.$order->id_customer.'" AND od.product_name LIKE "%Conditionnement : plant%" AND o.current_state NOT IN (1,6,7,8,10) AND od.id_order >= "223507";';
        ...
        //$req_dup1='SELECT DISTINCT od.id_order FROM `ps_order_detail` od JOIN ps_orders o on (od.id_order=o.id_order) WHERE o.id_order <> "'.$id_commande.'" AND o.id_customer = "'.$order->id_customer.'" AND od.product_name LIKE "%Conditionnement : plant%" AND o.current_state NOT IN (1,6,7,8,10) AND od.id_order >= "223507";';
