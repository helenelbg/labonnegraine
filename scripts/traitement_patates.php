...
die;
  ...
  $array_cmd = array();
  $histo = array();
  $patates_douces = array(1575,1576,2541,1940,3939,2542,2335,3860,2334);

  function dupliquer_commande($id_commande)
  {
    global $histo, $patates_douces;
    if ( !in_array($id_commande, $histo) )
    {
      ...
      {
        if ( in_array($unpdts_old['product_id'], $patates_douces) )
        {
      ...
      $req_dup_reg='SELECT DISTINCT od.id_order FROM `ps_order_detail` od JOIN ps_orders o on (od.id_order=o.id_order) WHERE o.id_customer = "'.$old_order->id_customer.'" AND o.id_order <> "'.$id_commande.'" AND od.product_id IN (1575,1576,2541,1940,3939,2542,2335,3860,2334) AND o.current_state NOT IN (1,6,7,8,10) AND o.date_add >="2024-04-22 12:30:00";';
      ...
      foreach($T_lignes_dup_reg as $T_ligne_commande_reg)
      {
        ...
        {
          if ( in_array($unpdts_old_reg['product_id'], $patates_douces) )
          {
	...
	$history->changeIdOrderState($etat_commande, $new_order->id);
      ...
      $req_dup_reg='SELECT DISTINCT od.id_order FROM `'.
              ...
              . ' WHERE o.id_customer = "'.$old_order->id_customer.'" AND o.id_order <> "'.$id_commande.'" AND o.id_order <> "'.$new_order->id.'" AND od.product_id IN (1575,1576,2541,1940,3939,2542,2335,3860,2334) AND o.current_state NOT IN (1,6,7,8,10) AND o.date_add >="2024-04-22 12:30:00";';
      $T_lignes_dup_reg = Db::getInstance()->executeS($req_dup_reg);
  ...
  $req_dup='SELECT DISTINCT od.id_order, o.id_customer FROM `ps_order_detail` od JOIN ps_orders o on (od.id_order=o.id_order) WHERE od.product_id IN (1575,1576,2541,1940,3939,2542,2335,3860,2334) AND o.current_state NOT IN (1,6,7,8,10) AND o.date_add >= "2024-04-22 12:30:00";';
  echo $req_dup.'<br />';
