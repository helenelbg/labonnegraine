...
class MySQL extends AbstractAdapter
{
    ...
    protected function computeWhereConditions(array $filterToTableMapping)
    {
        ...
        foreach ($this->getOperationsFilters() as $filterName => $filterOperations) {
            ...
            if (!empty($operationsConditions) && $operationIdx <= 3) { //SURCHARGE AW
	...
	//SURCHARGE AW
	$condaw = $joinMapping['joinCondition'];
	$condaw = str_replace("(p.id_product = fp_", "", $condaw);
	$condaw = str_replace(".id_product)", "", $condaw);

	if($joinMapping['tableName'] == "feature_product"){
		if($condaw <= 3){
