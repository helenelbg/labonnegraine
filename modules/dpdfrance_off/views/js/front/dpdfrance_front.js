...
/*const displayRelayBlock = () => {
    ...
    let relayDescriptionPart       = $("[data-id-carrier^="+dpdfranceRelaisCarrierId+"]:checked").parents('.delivery-option').children('label'); 

    let copyDPD = [];
    copyDPD.push(tableListPickUpPointsRelay);
    tableListPickUpPointsRelay.remove();

    for (var z = 0; z < relayDescriptionPart.length; z++) 
    {
        console.log($(relayDescriptionPart[z]));
        copyDPD.push(copyDPD.z);
        $(relayDescriptionPart[z]).appendTo(copyDPD.z);
        $(relayDescriptionPart[z]).appendTo('ICI '+z);
    }
...
jQuery(document).on('click', '#dpd-select-pickup-point', function () {
    console.log('la');

    let tableListPickUpPointsRelay = $('table#dpdfrance_relais_point_table.dpdfrance_fo');
    let relayDescriptionPart       = $(".modal-dpd-row-1");
...
const dpdFranceDisplayMethodBlock = () => {
    ...
    var carrierEC = '';
    for (var y = 0; y < selectedShippingMethod.length; y++) {
        if ( carrierEC != '' )
        {
            carrierEC += ',';
        }
        carrierEC += ''+$(selectedShippingMethod[y]).val().slice(0, -1);
    }


    //let selectedShippingMethodValue = selectedShippingMethod.val().slice(0, -1);
    let selectedShippingMethodValue = carrierEC;
    var splitC = selectedShippingMethodValue.split(",");
    var etatEC = false;

    if ( splitC.length > 1 && $('.multi-shipping-row').length == 0 )
    {

    }
    else 
    {
    for (var x = 0; x < splitC.length; x++) {
        console.log(splitC[x] +' === '+dpdfranceRelaisCarrierId);
        if (splitC[x] === dpdfranceRelaisCarrierId) {
            //displayRelayBlock();
            etatEC = true;
        } else if (splitC[x] === dpdfrancePredictCarrierId) {
            displayPredictBlock();
            etatEC = true;
        }
    }
