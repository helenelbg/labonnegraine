...
class SoNiceEtiquetageSessionFusion extends SoNice_Etiquetage
{
    ...
    public function fusion()
    {
        ...
        $id_fusion = Tools::getValue('id_fusion');

        if (!Validate::isInt($id_session)) {
            die($this->l('No session ID received.'));
        }
        if (!Validate::isInt($id_fusion)) {
            die($this->l('No ID for fusion received.'));
        }

        $result = SoColissimoSession::fusionSession($id_session, $id_fusion);
        if (!$result) {
            echo $this->l('An error occured during the session fusion.');
        }
...
$create = new SoNiceEtiquetageSessionFusion();
$create->fusion();
