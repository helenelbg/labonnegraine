...
class SoNiceEtiquetageDeleteLabel extends SoNice_Etiquetage
{
    ...
    public function delete()
    {
        ...
        $id_order = Tools::getValue('checkbox');

        if (!$id_order) {
            die($this->l('No id_order received.'));
        }

        $result = true;
        if (is_array($id_order) && count($id_order)) {
            foreach ($id_order as $id) {
                SoColissimoPDF::deleteLabelByIdOrder($id);
            }
        } else {
            $result = false;
        }
...
$create = new SoNiceEtiquetageDeleteLabel();
$create->delete();
