...
$(document).ready(function () {
    ...
    function get_labels() {
        ...
        var pAjax = {
            ...
            data_type: 'jsonp',
            data: selector_checkbox_checked.not(':disabled').serialize() + ($('input[name^="data"]:checked').length ? ('&' + $('input[name^="data"]:checked').serialize()) : '') + '&' + $('select[name^="nature"]').serialize() + '&context_key=' + $('#sne_context_key').val()
        };
    ...
    function printDocument(sne_print_list, printer_name) {
        ...
        if (print_type == 'PDF') {
            if ($('#merge_pdf').val() == 1) {
                // put the shifted id_order back in order_list
                order_list.unshift(id_order);

                var fn_url = $('#sne_pagination_url').val().split('pagination.php')[0] + 'merge_pdf_orders.php';
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (typeof(data) === 'undefined' || data === null) {
                            window.console && console.log(data);
                            $('#sne_error_orders').show();
                        }

                        if (typeof(data.url) !== 'string') {
                            plug.alert('Error with the PDF link, you may try without PDF Merger option.');
                        } else {
                            $('body').append('<iframe id="pdf_merged" style="display:none;"></iframe>');

                            $('#pdf_merged').attr('src', data.url).load(function () {
                    ...
                    error: function (data) {
                        window.console && console.log(data);
                        $('#sne_error_orders').show().find('code').html(data.responseText);
                    }
                });
            } else {
            ...
            if (typeof(CommonPrintServer) == 'object' && typeof(CommonPrintServer.getPrinters) == 'function') {
                CommonPrintServer.setPrinter(printer_name, function () {
                    CommonPrintServer.printFileByURL(pdf_url, function (data) {
    ...
    $('#generate_slip').click(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (typeof(data.result) === 'undefined' || data.result === null || data.result === false) {
                    window.console && console.log(data);
                    $('#sne_error_close').show().find('code').html('Erreur !' + (data.console !== '' ? '<br>' + data.console : ''));
                    return;
                }
    ...
    selector_table_orders.delegate('.ta', 'click', function () {
        var selector_rno = $(this).closest('tr').find('input[type="radio"]');
        selector_rno.is(':checked') && selector_rno.attr('checked', false);
    });
