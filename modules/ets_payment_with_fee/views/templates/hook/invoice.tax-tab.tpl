...
{if $isTaxEnabled}
    {if $tax_exempt}
        ...
        {l s='Exempt of VAT according to section 259B of the General Tax Code.' mod='ets_payment_with_fee' pdf='true'}
    {elseif (isset($tax_breakdowns) && $tax_breakdowns)}
        <table id="tax-tab" width="100%">
            <thead>
            <tr>
                <th class="header small">{l s='Tax Detail' mod='ets_payment_with_fee' pdf='true'}</th>
                <th class="header small">{l s='Tax Rate' mod='ets_payment_with_fee' pdf='true'}</th>
                {if $display_tax_bases_in_breakdowns}
                    <th class="header small">{l s='Base price' mod='ets_payment_with_fee' pdf='true'}</th>
                {/if}
                <th class="header-right small">{l s='Total Tax' mod='ets_payment_with_fee' pdf='true'}</th>
            </tr>
            </thead>
            <tbody>
                        ...
                        <td class="white">
                            {if !$label_printed}
                                {if $label == 'product_tax'}
                                    {l s='Product' mod='ets_payment_with_fee' pdf='true'}
                                {elseif $label == 'shipping_tax'}
                                    {l s='Shipping' mod='ets_payment_with_fee' pdf='true'}
                                {elseif $label == 'ecotax_tax'}
                                    {l s='Ecotax' mod='ets_payment_with_fee' pdf='true'}
                                {elseif $label == 'wrapping_tax'}
                                    {l s='Wrapping' mod='ets_payment_with_fee' pdf='true'}
                                {elseif $label == 'payment_fee'}
                                    {l s='Payment fee' mod='ets_payment_with_fee' pdf='true'}
                                {/if}
            ...
            {if !$has_line}
                <tr>
                    <td class="white center" colspan="{if $display_tax_bases_in_breakdowns}4{else}3{/if}">
                        {l s='No taxes' mod='ets_payment_with_fee' pdf='true'}
                    </td>
                </tr>
            {/if}
