...
class ColissimoService extends ObjectModel
{
    ...
    {
        if ( $_SERVER['REMOTE_ADDR'] == '176.134.93.165' || $_SERVER['REMOTE_ADDR'] == '80.15.118.113' )
            {
                //error_log('isEligibleToAccount 0 : '.$isoCountryCustomer . ' // '.print_r($accountType, true));
            }
        ...
        if (in_array($this->destination_type, $destinationWorldWideEU) && !isset($accountType[$this->destination_type])) {
            if ( $_SERVER['REMOTE_ADDR'] == '176.134.93.165' || $_SERVER['REMOTE_ADDR'] == '80.15.118.113' )
            {
                error_log('isEligibleToAccount 1 : '.$isoCountryCustomer . ' // '.print_r($accountType, true));
            }
            return false;
        }
        //@formatter:on
        if ($this->type == ColissimoService::TYPE_RELAIS) {
            if (!in_array($isoCountryCustomer, $availableIso)) {
                if ( $_SERVER['REMOTE_ADDR'] == '176.134.93.165' || $_SERVER['REMOTE_ADDR'] == '80.15.118.113' )
            {
                error_log('isEligibleToAccount 2 : '.$isoCountryCustomer . ' // '.print_r($accountType, true));
            }
                return false;
            }
        }
        if ($this->destination_type == ColissimoTools::DEST_EU &&
            ...
            if (!in_array($isoCountryCustomer, ColissimoTools::$isoEUCountriesZone1Zone3)) {
                if ( $_SERVER['REMOTE_ADDR'] == '176.134.93.165' || $_SERVER['REMOTE_ADDR'] == '80.15.118.113' )
            {
                error_log('isEligibleToAccount 3 : -'.$this->destination_type.'- '.$isoCountryCustomer . ' // '.print_r(ColissimoTools::$isoEUCountriesZone1Zone3, true));
            }
                return false;
            }
        }
        if ($this->destination_type == ColissimoTools::DEST_OM) {
            if (!isset($accountType[$this->destination_type])) {
                if ( $_SERVER['REMOTE_ADDR'] == '176.134.93.165' || $_SERVER['REMOTE_ADDR'] == '80.15.118.113' )
            {
                error_log('isEligibleToAccount 4 : '.$isoCountryCustomer . ' // '.print_r($accountType, true));
            }
                return false;
            }
            if ($this->product_code == 'ECO' &&
                ...
                {
                    error_log('isEligibleToAccount 5 : '.$isoCountryCustomer . ' // '.print_r($accountType, true));
                }
                return false;
            }
        }
        if ( $_SERVER['REMOTE_ADDR'] == '176.134.93.165' || $_SERVER['REMOTE_ADDR'] == '80.15.118.113' )
        {
           // error_log('isEligibleToAccount 6 : '.$isoCountryCustomer . ' // '.print_r($accountType, true));
        }
        return true;
    }
