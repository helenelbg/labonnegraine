		...
		<div class="form-group">
			<div class="col-lg-12" id="{$id|escape:'html':'UTF-8'}-images-thumbnails">
				{foreach $files as $file}
					{if isset($file.image) && $file.type == 'image'}
						<div>
							{$file.image  nofilter}{* HTML, cannot escape *}
							{if isset($file.size)}<p>{l s='File size' mod='thegiftcard'} {$file.size|floatval}kb</p>{/if}
							{if isset($file.delete_url)}
								<p>
									<a class="btn btn-default" href="{$file.delete_url|escape:'html':'UTF-8'}">
										<i class="icon-trash"></i> {l s='Delete' mod='thegiftcard'}
									</a>
								</p>
							{/if}
						</div>
...
{if isset($max_files) && $files|count >= $max_files}
	<div class="row">
		<div class="alert alert-warning">
			{l s='You have reached the limit (%s) of files to upload, please remove files to continue uploading' sprintf=$max_files mod='thegiftcard'}
		</div>
	</div>
{else}
	<div class="form-group">
		<div class="col-sm-6">
			<input id="{$id|escape:'html':'UTF-8'}" type="file" name="{$name|escape:'html':'UTF-8'}"
				{if isset($multiple) && $multiple} multiple="multiple" {/if} class="hide" />
				...
				<span class="input-group-btn">
					...
					{if (!isset($multiple) || !$multiple) && isset($files) && $files|count == 1 && isset($files[0].download_url)}
						<a href="{$files[0].download_url|escape:'html':'UTF-8'}" class="btn btn-default">
							...
							{if isset($size)}{l s='Download current file (%skb)' sprintf=$size mod='thegiftcard'}{else}{l s='Download current file' mod='thegiftcard'}{/if}
						</a>
					{/if}
				</span>
			</div>
