...
class Scalapay extends PaymentModule
{
    ...
    protected function getPluginVersion()
    {
        ...
        // @phpstan-ignore-next-line
        $this->version = (string)file_get_contents(_PS_MODULE_DIR_ . $this->name . "/VERSION.txt");
    ...
    public function install()
    {
        ...
        if (version_compare(_PS_VERSION_, "1.7.0", "<")) {
            ...
            Configuration::updateValue(self::SCALAPAY_PAY_IN_3_CHECKOUT_PAGE_CUSTOM_CSS, 'margin-left:150px; margin-bottom:20px;');
        } else {
            ...
            Configuration::updateValue(self::SCALAPAY_PAY_IN_3_CHECKOUT_PAGE_CUSTOM_CSS, 'margin-bottom:20px;');
        }
        ...
        if (version_compare(_PS_VERSION_, "1.7.0", "<")) {
            ...
            Configuration::updateValue(self::SCALAPAY_PAY_IN_4_CHECKOUT_PAGE_CUSTOM_CSS, 'margin-left:150px; margin-bottom:20px;');
        } else {
            ...
            Configuration::updateValue(self::SCALAPAY_PAY_IN_4_CHECKOUT_PAGE_CUSTOM_CSS, 'margin-bottom:20px;');
        }
        ...
        if (version_compare(_PS_VERSION_, "1.7.0", "<")) {
            ...
            Configuration::updateValue(self::SCALAPAY_PAY_LATER_CHECKOUT_PAGE_CUSTOM_CSS, 'margin-left:150px; margin-bottom:20px;');
        } else {
            ...
            Configuration::updateValue(self::SCALAPAY_PAY_LATER_CHECKOUT_PAGE_CUSTOM_CSS, 'margin-bottom:20px;');
        }
    ...
    public function installOrderState()
    {
        ...
        $order_state->name = array(
            $this->context->language->id => "Waiting Scalapay capture",
        );
...
//            }
    ...
    {
        $data = array(
            "online_later" => array("min" => 5, "max" => 899),
            "online_pay-in-4" => array("min" => 900, "max" => 2000),
            "online_pay-in-3" => array("min" => 5, "max" => 899),
        );
        ...
        return array_map(function ($v) {
            return array("min" => $v["configuration"]["minimumAmount"]["amount"], "max" => $v["configuration"]["maximumAmount"]["amount"]);
...
//                    ],
    ...
    {
        $languages = Language::getLanguages(false) ?: array();
        $config = array();
    ...
    protected function postProcess()
    {
        ...
        foreach ($this->configs as $key) {
            switch ($key) {
                ...
                case self::SCALAPAY_PAY_LATER_ALLOWED_CURRENCIES:
                case self::SCALAPAY_PAY_LATER_ALLOWED_LANGUAGES:
                    Configuration::updateValue($key, implode(',', Tools::getvalue($key) ?: array()));
                    break;
                case self::SCALAPAY_PAY_IN_3_RESTRICTED_CATEGORIES:
                case self::SCALAPAY_PAY_IN_4_RESTRICTED_CATEGORIES:
                case self::SCALAPAY_PAY_LATER_RESTRICTED_CATEGORIES:
                    Configuration::updateValue($key, json_encode(Tools::getvalue($key) ?: array()));
                    break;
                case self::SCALAPAY_PAY_IN_3_PRODUCT_PAGE_AFTER_WIDGET_TEXT:
                case self::SCALAPAY_PAY_IN_4_PRODUCT_PAGE_AFTER_WIDGET_TEXT:
    ...
    public function hookDisplayPayment()
    {
        $this->context->smarty->assign(array(
            "scalapayPayment" => array(
                "logo" => _PS_BASE_URL_ . __PS_BASE_URI__ . 'modules/scalapay/views/img/payment.jpg',
    ...
    public function hookActionPresentPaymentOptions($params)
    {
        $params["paymentOptions"] = array_merge(
            ...
            $this->createPaymentOptions($this->getPaymentInfos())
        );
    }
    ...
    public function hookActionAdminControllerSetMedia()
    {
        ...
        if ($controller == "AdminModules"
            && Tools::getValue('configure') == 'scalapay') {
    ...
    public function displayWidget()
    {
        ...
        $this->context->smarty->assign(array('scalapay_in_page_checkout' => $config));
    ...
    {
        if (!$this->isPayIn3Enabled(array($product->id))
    ...
    {
        if (!$this->isPayIn4Enabled(array($product->id))
    ...
    {
        if (!$this->isPayLaterEnabled(array($product->id))
    ...
    protected function areProductsEnabledForCategories($categoryIds = array(), $productIds = array())
    {
    ...
    protected function buildCategoryHierarchy($categories = array(), $currentCategoryId = 0)
    {
        $hierarchyCategories = array();
    ...
    protected function payIn3PaymentOption()
    {
        ...
        if (!$this->isPayIn3Enabled(array_map(function (array $product) {
            ...
            || !in_array($deliveryAddressCountryIso, explode(",", Configuration::get(self::SCALAPAY_PAY_IN_3_ALLOWED_COUNTRIES)))
        ) {
            return null;
        }

        $templateData = array(
            "scalapayWidget" => array(
        ...
        return array(
            "action" => $this->context->link->getModuleLink($this->name, 'validation', array("product" => self::PRODUCT_PAY_IN_3), true),
            "callToActionText" => sprintf($this->l('Pay in %s installments'), 3),
            "logo" => _MODULE_DIR_ . $this->name . '/views/img/scalapay-logo-black.png',
    ...
    protected function payIn4PaymentOption()
    {
        ...
        return array(
            "action" => $this->context->link->getModuleLink($this->name, 'validation', array("product" => self::PRODUCT_PAY_IN_4), true),
    ...
    protected function payLaterPaymentOption()
    {
        ...
        return array(
            "action" => $this->context->link->getModuleLink($this->name, 'validation', array("product" => self::PRODUCT_PAY_LATER), true),
