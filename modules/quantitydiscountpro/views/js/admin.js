...
$("[id='action_condition_group_price["+id+"]']").show();
            $("[id='action_condition_filters["+id+"]']").show();
            if ($("input[name='action_filter_by_category["+id+"]']:checked").val() == 0) {
                $("[id='action_condition_default_category["+id+"]']").hide();
            } else {
...
function displayProductAttributes(id) {
    if ($("[id='ipa_" + $("[name='action_gift_product\["+id+"\]']").val() + "["+id+"]'] option").length === 0) {
        ...
        $("[id='ipa_" + $("[name='action_gift_product\["+id+"\]']").val() + "["+id+"]']").show();
    }
}
...
jQuery.fn.filterByText = function(textbox, selectSingleMatch) {
    return this.each(function() {
        var select = $(this);
        var selectOptions = $('#' + $(this).attr('id') + ' option');
        var options = [];
...
function countMultiselectElements() {
    $('select[multiple]').each(function() {
        console.log($(this).attr('id')+' has '+ $('#' + $(this).attr('id') + ' option').size() +' elements');
    })
}
