...
<div class="alert alert-warning" id="{$list_id|escape:'htmlall':'UTF-8'}-empty-filters-alert" style="display:none;">{l s='Please fill at least one field to perform a search in this list.' mod='dmulistecommandes'}</div>
{if isset($sql) && $sql}
...
{if !$simple_header}
	...
	<div class="panel col-lg-12">
		<div class="panel-heading">
			{if isset($icon)}<i class="{$icon|escape:'htmlall':'UTF-8'}"></i> {/if}{if is_array($title)}{$title|end|escape:'htmlall':'UTF-8'}{else}{$title|escape:'htmlall':'UTF-8'}{/if}
			{if isset($toolbar_btn) && count($toolbar_btn) >0}
				...
				{foreach from=$toolbar_btn item=btn key=k}
					{if $k != 'modules-list' && $k != 'back'}
						<a id="desc-{$table|escape:'htmlall':'UTF-8'}-{if isset($btn.imgclass)}{$btn.imgclass|escape:'htmlall':'UTF-8'}{else}{$k|escape:'htmlall':'UTF-8'}{/if}" class="list-toolbar-btn{if isset($btn.target) && $btn.target} _blank{/if}"{if isset($btn.href)} href="{$btn.href|escape:'html':'UTF-8'}"{/if}{if isset($btn.js) && $btn.js} onclick="{$btn.js|escape:'htmlall':'UTF-8'}"{/if}>
				...
				{/foreach}
					<a class="list-toolbar-btn" href="javascript:location.reload();">
						<span title="" data-toggle="tooltip" class="label-tooltip" data-original-title="{l s='Refresh list' mod='dmulistecommandes'}" data-html="true" data-placement="top">
				...
				{if isset($sql) && $sql}
					...
					{if $sql_manager.view == 1}
						<a class="list-toolbar-btn" href="javascript:void(0);" onclick="$('.leadin').first().append('<div class=\'alert alert-info\'>' + $('#sql_query_{$list_id|escape:'html':'UTF-8'}').val() + '</div>'); $(this).attr('onclick', '');">
							<span class="label-tooltip" data-toggle="tooltip" data-original-title="{l s='Show SQL query' mod='dmulistecommandes'}" data-html="true" data-placement="top" >
						...
						<a class="list-toolbar-btn" href="javascript:void(0);" onclick="$('#sql_name_{$list_id|escape:'html':'UTF-8'}').val(createSqlQueryName()); $('#sql_query_{$list_id|escape:'html':'UTF-8'}').val($('#sql_query_{$list_id|escape:'html':'UTF-8'}').val().replace(/\s+limit\s+[0-9,\s]+$/ig, '').trim()); $('#sql_form_{$list_id|escape:'html':'UTF-8'}').submit();">
							<span class="label-tooltip" data-toggle="tooltip" data-original-title="{l s='Export to SQL Manager' mod='dmulistecommandes'}" data-html="true" data-placement="top" >
					...
					if (btn_save.length > 0 && btn_submit.length > 0) {
						...
						if (btn_save_and_stay.length > 0) {
							...
							if (btn_submit.val().length > 0 && lbl_save_and_stay && !lbl_save_and_stay.hasClass('locked')) {
								lbl_save_and_stay.html(btn_submit.val() + " {l s='and stay' mod='dmulistecommandes'} ");
							}
						}
				...
				<tr class="nodrag nodrop">
					...
					<th class="{if isset($params.class)}{$params.class|escape:'htmlall':'UTF-8'}{/if}{if isset($params.align)} {$params.align|escape:'htmlall':'UTF-8'}{/if}">
						<span class="title_box{if isset($order_by) && ($key == $order_by)} active{/if}">
							...
							{else}
								{$params.title|escape:'quotes':'UTF-8'}
							{/if}
					...
					{if $shop_link_type}
						<th>
							...
							{if $shop_link_type == 'shop'}
								{l s='Shop' mod='dmulistecommandes'}
							{else}
								{l s='Shop group' mod='dmulistecommandes'}
							{/if}
							</span>
						</th>
					{/if}
					...
					{foreach $fields_display AS $key => $params}
						<th {if isset($params.align)} class="{$params.align|escape:'htmlall':'UTF-8'}" {/if}>
                          ...
                          name="{$list_id|escape:'htmlall':'UTF-8'}Filter_{if isset($params.filter_key)}{$params.filter_key|escape:'htmlall':'UTF-8'}{else}{$key|escape:'htmlall':'UTF-8'}{/if}">
										<option value="">-</option>
										<option value="1" {if $params.value == 1} selected="selected" {/if}>{l s='Yes' mod='dmulistecommandes'}</option>
										<option value="0" {if $params.value == 0 && $params.value != ''} selected="selected" {/if}>{l s='No' mod='dmulistecommandes'}</option>
									</select>
								{elseif $params.type == 'date' || $params.type == 'datetime'}
									<div class="date_range row">
 										<div class="input-group fixed-width-md center">
											<input type="text" class="filter datepicker date-input form-control" id="local_{$params.id_date|escape:'htmlall':'UTF-8'}_0" name="local_{$params.name_date|escape:'htmlall':'UTF-8'}[0]"  placeholder="{l s='From' mod='dmulistecommandes'}" />
 										...
 										<div class="input-group fixed-width-md center">
											<input type="text" class="filter datepicker date-input form-control" id="local_{$params.id_date|escape:'htmlall':'UTF-8'}_1" name="local_{$params.name_date|escape:'htmlall':'UTF-8'}[1]"  placeholder="{l s='To' mod='dmulistecommandes'}" />
								...
								{elseif $params.type == 'select'}
									...
									{/if}
								{else}
									<input placeholder="{if ($key == 'total_paid_tax_incl')}{l s='Total' mod='dmulistecommandes'}{else}{$params.title|escape:'htmlall':'UTF-8'}{/if}" type="text" class="filter" name="{$list_id|escape:'htmlall':'UTF-8'}Filter_{if isset($params.filter_key)}{$params.filter_key|escape:'htmlall':'UTF-8'}{else}{$key|escape:'htmlall':'UTF-8'}{/if}" value="{$params.value|escape:'html':'UTF-8'}" {if isset($params.width) && $params.width != 'auto'} style="width:{$params.width|escape:'htmlall':'UTF-8'}px"{/if} />
                                    {if ($key == 'total_paid_tax_incl')}
                                        <input placeholder="{l s='Product' mod='dmulistecommandes'}" type="text" class="filter" name="{$list_id|escape:'htmlall':'UTF-8'}Filter_product" value="" style="margin-top:5px" />
                                    {/if}
								{/if}
							{/if}
						</th>
						...
						<th class="actions">
							...
							<span class="pull-right">
								...
								<button type="submit" id="submitFilterButton{$list_id|escape:'htmlall':'UTF-8'}" name="submitFilter" class="btn btn-default" data-list-id="{$list_id|escape:'htmlall':'UTF-8'}">
									<i class="icon-search"></i> {l s='Search' mod='dmulistecommandes'}
								</button>
								{if $filters_has_value}
									<button type="submit" name="submitReset{$list_id|escape:'htmlall':'UTF-8'}" class="btn btn-warning">
										<i class="icon-eraser"></i> {l s='Reset' mod='dmulistecommandes'}
									</button>
								{/if}
							</span>
							{/if}
