...
class AdminStoreCommander extends ModuleAdminController
{
    ...
    public function renderView()
    {
        ...
        if (Tools::getIsset("SETLOGUSER")) {
            ...
            if (version_compare(_PS_VERSION_, '1.5.0.0', '>=')) {
                ...
                if ($id_shop == 0 && Shop::getTotalShops() > 1) {
                    $this->errors[] = 'There is a problem with the shop ID';
                } else {
        ...
        elseif(version_compare(_PS_VERSION_, '1.7.0.0', '>=') && Tools::getIsset("REDIRECTADMIN")){
            ...
            $scdir = Configuration::getGlobalValue('SC_FOLDER_HASH');
            if ($scdir === false) {
                $this->errors[] = 'Unable to find the Store Commander folder. Please contact <a href="support.storecommander.com" targe="_blank">support.storecommander.com</a>';
            } else {
                $sc_title = "Loading...";
                $sc_url = '../modules/storecommander/' . $scdir . '/'.$this->setScDirByVersion($scdir).'/index.php?ide=' . $cookie->id_employee . '&psap=' . $psadminpath . '&key=' . md5($cookie->id_employee . $datelastregen) . (version_compare(_PS_VERSION_,'1.4.0.0', '>=') ? '' : '&id_lang=' . $cookie->id_lang);
            }
        }
    ...
    private function setScDirByVersion($scFolder = null)
    {
        if(!$scFolder) {
            $scFolder = Configuration::getGlobalValue('SC_FOLDER_HASH');
        }
        if(version_compare(PHP_VERSION, '8.0.0', '>=') &&
            version_compare(_PS_VERSION_, '8.0.0', '>=') &&
            file_exists(_PS_MODULE_DIR_.'storecommander/'.$scFolder.'/SCR/'))
        {
            return 'SCR';
        }
        return 'SC';
