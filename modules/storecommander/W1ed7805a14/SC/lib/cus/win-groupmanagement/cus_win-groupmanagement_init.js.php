<?php echo '<script type="text/javascript">'; ?>
    ...
    wGroupManagement.tb.setIconset('awesome');
    wGroupManagement.tb.addButton("del_group", 0, "", "fa fa-minus-circle red", "fa fa-minus-circle red");
    wGroupManagement.tb.setItemToolTip('del_group','<?php echo _l('Delete selected groups'); ?>');
    wGroupManagement.tb.addButton("selectall", 0, "", "fa fa-bolt yellow", "fa fa-bolt yellow");
    wGroupManagement.tb.setItemToolTip('selectall','<?php echo _l('Select all groups'); ?>');
    wGroupManagement.tb.addButton("add_group", 0, "", "fa fa-plus-circle green", "fa fa-plus-circle green");
    wGroupManagement.tb.setItemToolTip('add_group','<?php echo _l('Create a new group'); ?>');
    wGroupManagement.tb.addButton("refresh", 0, "", "fa fa-sync green", "fa fa-sync green");
    ...
    wGroupManagement.tb.attachEvent("onClick",
        function(id){
            ...
            if (id=='del_group')
            {
                if (confirm('<?php echo _l('Permanently delete the selected groups everywhere in the shop.', 1); ?>'))
                {
    ...
    cus_group_prop_tb.setIconset('awesome');
    actual_groupmanagement_subproperties = "cus_group_categories";
    var opts = new Array();
    cus_group_prop_tb.addButtonSelect("cus_group_prop_subproperties", 100, "<?php echo _l('Categories sharing'); ?>", opts, "fa fa-folder yellow", "fa fa-folder yellow",false,true);
    ...
    foreach ($sub_files as $sub_item)
    {
        ...
        {
            if (is_dir(SC_DIR.'lib/cus/win-groupmanagement/'.$sub_item) && file_exists(SC_DIR.'lib/cus/win-groupmanagement/'.$sub_item.'/cus_win-groupmanagement_'.$sub_item.'_init.js.php'))
            {
                require_once SC_DIR.'lib/cus/win-groupmanagement/'.$sub_item.'/cus_win-groupmanagement_'.$sub_item.'_init.js.php';
            }
            elseif (is_dir(SC_DIR.'lib/cus/win-groupmanagement/'.$sub_item) && file_exists(SC_DIR.'lib/cus/win-groupmanagement/'.$sub_item.'/cus_win-groupmanagement_'.$sub_item.'_init.php'))
            {
    ...
    function onGridCusGroupContextButtonClick(itemId) {
        ...
        if (itemId=="gopsbo"){
            ...
            wModifyGroup.attachURL("<?php echo SC_PS_PATH_ADMIN_REL; ?>index.php?<?php echo version_compare(_PS_VERSION_, '1.5.0.0', '>=') ? 'controller=AdminGroups' : 'tab=AdminGroups'; ?>&id_group="+tabId+"&updategroup&id_lang="+SC_ID_LANG+"&adminlang=1&token=<?php echo $sc_agent->getPSToken('AdminGroups'); ?>");
...
function displayGroupsList()
{
    ...
    wGroupManagement.gridGroups.load("index.php?ajax=1&act=cus_win-groupmanagement_get&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),function()
    {
