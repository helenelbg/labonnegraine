...
?>
    ...
    lastCustomerSelID=0;
    propertiesPanel='<?php echo _s('CUS_CUSPROP_GRID_DEFAULT'); ?>';
...
<?php
    ...
    cus_shoptree.setImagePath('lib/js/imgs/dhxtree_material/');
    ...
    var cusShoptreeTB = cus_storePanel.attachToolbar();
      cusShoptreeTB.setIconset('awesome');
    cusShoptreeTB.addButton("help", 0, "", "fad fa-question-circle blue", "fad fa-question-circle blue");
    ...
    {
        let allShops_item_found = cus_shoptree.getIndexById('all');
        if(allShops_item_found !== null) {
            var children = cus_shoptree.getAllSubItems("all").split(",");
        } else {
            var children = cus_shoptree.getAllChildless().split(",");
        }
    ...
    function saveCheckSelection()
    {
        ...
        if(shopselection=="all" || shopselection=="0")
        {
            ...
            if(allShops_item_found !== null) {
                ...
                checked = cus_shoptree.getAllChildless();
            }
        }
    ...
    function displayShopTree(callback) {
        ...
        cus_shoptree.load("index.php?ajax=1&act=cus_shop_get&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),function(){
...
{ ?>
    ...
    cat_tb.addButton("group_add", 0, "", "fa fa-plus-circle green", "fa fa-plus-circle green");
    ...
    cat_tb.attachEvent("onClick",
        function(id){
            if (id=='help'){
                <?php echo "window.open('".getScExternalLink('support_customers_group')."');"; ?>
            }
            if (id=='refresh'){
                displayFilters();
            }
            if(id=='group_add'){
                ...
                if (gname!=null)
                    $.post("index.php?ajax=1&act=cus_group_update&action=insert&id_lang="+SC_ID_LANG,{name:gname},function(id){
                        if(id!=null && id!==0 && id!=="")
                        {
                            displayFilters();
                        }
...
?>
    ...
    cus_filter.setImagePath('lib/js/imgs/dhxtree_material/');
...
?>
    ...
    function displayFilters(callback)
    {
        ...
        cus_filter.load("index.php?ajax=1&act=cus_filter_get&id_lang="+SC_ID_LANG+"&id_shop="+shopselection+"&"+new Date().getTime(),function(){
                if (groupselection!=0)
                {
