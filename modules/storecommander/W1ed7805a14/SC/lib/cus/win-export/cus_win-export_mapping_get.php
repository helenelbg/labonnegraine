...
$data = ExportCustomerMapping::getMappingList();
$xml = array();
if ($data)
{
    foreach ($data as $row)
    {
        ...
        $xml[] = '<row id="'.(int) $row[ExportCustomerMapping::$definition['primary']].'">'.implode("\r\n\t", $row_xml).'</row>';
    }
}
$xml = implode("\r\n", $xml);
...
<rows id="0">
    <head>
        ...
        <column id="<?php echo ExportCustomerMapping::$definition['primary']; ?>" width="40" type="ro" align="left" sort="int" hidden="true"><?php echo _l('ID'); ?></column>
