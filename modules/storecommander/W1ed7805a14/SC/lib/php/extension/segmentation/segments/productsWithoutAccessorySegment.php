...
class productsWithoutAccessorySegment extends SegmentCustom
{
    public $name = 'Products without accessory';
    ...
    public function _executeHook_segmentAutoSqlQueryGrid($name, $params = array())
    {
        ...
        WHERE id_product NOT IN (SELECT DISTINCT(id_product_1) FROM '._DB_PREFIX_.'accessory)'.
    ...
    public function _executeHook_segmentAutoSqlQuery($name, $params = array())
    {
        ...
        $where = ' '.(empty($params['no_operator']) ? 'AND' : '').' ( p.id_product NOT IN (SELECT DISTINCT(id_product_1) FROM '._DB_PREFIX_.'accessory) '.
