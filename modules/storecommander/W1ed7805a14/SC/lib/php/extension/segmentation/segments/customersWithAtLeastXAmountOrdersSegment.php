...
{
    public $name = 'Customers according to ordered amount';
    ...
    public function _executeHook_segmentAutoConfig($name, $params = array())
    {
        ...
        $html = '<strong>'._l('Format: 0.00').' '._l('(store default currency)').'</strong>
        <br/> <br/>'.
        _l('Amount').' <select name="operator" style="width: 15%">
        ...
        <input type="text" id="x_amount" name="x_amount" style="width: 20%;" value="'.(!empty($values['x_amount']) ? $values['x_amount'] : '').'" />
        <br/><strong>'._l('OR').'</strong><br/>
        '._l('Amount between').' <input type="text" id="between_a" name="between_a" style="width: 20%;" value="'.(!empty($values['between_a']) ? $values['between_a'] : '').'" /> 
    ...
    public function _executeHook_segmentAutoSqlQueryGrid($name, $params = array())
    {
        ...
        if (!empty($params['auto_params']))
        {
            ...
            $operation = '';
            if (!empty($auto_params['x_amount']))
            {
                if ($auto_params['operator'] == 'supp')
                {
                    $operation = " > '".(int) $auto_params['x_amount']."' ";
                }
            ...
            if (!empty($operation))
            {
                $sql = 'SELECT c.id_customer, SUM(o.total_paid) AS amount_orders
                    ...
                    HAVING amount_orders '.$operation;
    ...
    public function _executeHook_segmentAutoSqlQuery($name, $params = array())
    {
        ...
        if (!empty($params['auto_params']))
        {
            ...
            if (!empty($operation))
            {
                $where = ' '.(empty($params['no_operator']) ? 'AND' : '').'  (SELECT SUM(o2.total_paid) AS amount_orders
