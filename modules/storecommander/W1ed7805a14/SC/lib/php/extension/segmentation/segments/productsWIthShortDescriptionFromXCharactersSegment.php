...
class productsWIthShortDescriptionFromXCharactersSegment extends SegmentCustom
{
    public $name = 'Products with a short description of less, more or equal to X characters';
    ...
    public function _executeHook_segmentAutoConfig($name, $params = [])
    {
        ...
        $html = '<select name="operator" style="width: 15%">';
        ...
        <input type="number" id="x_characters" name="x_characters" style="width: 40%;" value="'.(!empty($values['x_characters']) ? $values['x_characters'] : '').'" />
    ...
    protected function getMainDbQuery($segmentParams = [])
    {
        ...
        $whereCondition = [
            ...
            'plang.`description_short` <> ""',
            'plang.`description_short` IS NOT NULL',
        ];
        ...
        $productsQuery
            ->select('DISTINCT plang.id_product, description_short')
        ...
        if ($res){
            foreach($res as $row)
            {
                if($this->needInsertId($operator, $nbCharacters, $row['description_short']))
                {
