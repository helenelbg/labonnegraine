...
$exportId = Tools::getValue(ExportOrder::$definition['primary'],null);
switch ($action)
{
    case 'add_export':
        $exportObject = new ExportOrder();
        ...
        break;
    case 'delete_export':
        $exportObject = new ExportOrder($exportId);
        if(!$exportObject->id){
    ...
    case 'reset_token':
        ...
        {
            $exportObject = new ExportOrder($id_export);
            if(!$exportObject->id)
            {
                continue;
            }
