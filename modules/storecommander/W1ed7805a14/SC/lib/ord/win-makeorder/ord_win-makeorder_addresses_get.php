...
if (SCMS && $id_shop > 0)
{
    $inner = ' INNER JOIN '._DB_PREFIX_."country_shop gs ON (gs.id_country = g.id_country AND gs.id_shop = '".(int) $id_shop."') ";
}
...
if (SCMS && $id_shop > 0)
{
    $inner = ' INNER JOIN '._DB_PREFIX_."country_shop cs ON (cs.id_country = g.id_country AND cs.id_shop = '".(int) $id_shop."') ";
}
...
{
    global $id_customer,$id_shop,$id_cart;

    $sql = 'SELECT * FROM '._DB_PREFIX_."cart WHERE id_cart = '".(int) $id_cart."'";
    ...
    if (SCMS && $id_shop > 0)
    {
        $sql .= ' INNER JOIN '._DB_PREFIX_."customer c ON (a.id_customer = c.id_customer AND c.id_shop = '".(int) $id_shop."') ";
    }
    $sql .= ' WHERE a.active = 1 
                    AND a.deleted = 0 
                    AND a.id_customer IN ('.pInSQL($id_customer).')';
    ...
    if (!empty($res))
    {
        foreach ($res as $address)
        {
            ...
            $xml .= '<cell><![CDATA['.(!empty($cart['id_address_invoice']) && $cart['id_address_invoice'] == $address['id_address'] ? '1' : '0').']]></cell>';
            $xml .= '<cell><![CDATA['.(!empty($cart['id_address_delivery']) && $cart['id_address_delivery'] == $address['id_address'] ? '1' : '0').']]></cell>';
...
<rows id="0">
    <head>
        <beforeInit>
            <call command="attachHeader"><param><![CDATA[,,#numeric_filter,#text_filter,#text_filter,#text_filter,#text_filter,<?php if (_s('CUS_USE_COMPANY_FIELDS')){ ?>#text_filter,<?php } ?>#text_filter,#text_filter,#text_filter,#text_filter,#select_filter,#select_filter,#select_filter]]></param></call>
        </beforeInit>
        <column id="invoice" width="80" type="ra" align="center" sort="str"><?php echo _l('Invoice'); ?></column>
        <column id="delivery" width="70" type="ra" align="center" sort="str"><?php echo _l('Delivery'); ?></column>
        ...
        <column id="id_state" width="70" type="coro" align="left" sort="str"><?php echo _l('State'); ?>
            <?php foreach ($arrStates as $id => $state)
{
    echo '<option value="'.$id.'"><![CDATA['.$state.']]></option>';
}?>
        </column>
        <column id="id_country" width="70" type="coro" align="left" sort="str"><?php echo _l('Country'); ?>
            <?php foreach ($arrCountrys as $id => $country)
{
    echo '<option value="'.$id.'"><![CDATA['.$country.']]></option>';
}?>
        </column>
    </head>
    <?php
