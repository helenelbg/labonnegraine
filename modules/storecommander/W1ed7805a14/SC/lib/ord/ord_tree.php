...
<script type="text/javascript">
    ...
    layoutStatusText = "<?php echo SC_COPYRIGHT.' '.(SC_DEMO ? '- Demonstration' : '- '._l('License').' '.SCLIMREF.' - '.$SC_SHOP_PRODUCTSCOUNT.' '._l('products')).' - Version '.SC_VERSION.(SC_BETA ? ' BETA' : '').(SC_GRIDSEDITOR_INSTALLED ? ' GE'.(SC_GRIDSEDITOR_PRO_INSTALLED ? 'P' : '') : '').' (PS '._PS_VERSION_.(defined('_PS_CACHE_ENABLED_') && _PS_CACHE_ENABLED_ ? ' (cache)' : '').') '.$NOTEPAD_BUTTON; ?>";
...
?>
    ...
    clipboardValue=null;
    clipboardType=null;
    var period_window_width = 350;
    var period_window_height = 230;
...
<?php
    ...
    ord_shoptree.setImagePath('lib/js/imgs/dhxtree_material/');
    ord_shoptree.enableSmartXMLParsing(true);

    var ordShoptreeTB = ord_storePanel.attachToolbar();
      ordShoptreeTB.setIconset('awesome');
    ordShoptreeTB.addButton("help", 0, "", "fad fa-question-circle blue", "fad fa-question-circle blue");
    ...
    function displayShopTree(callback) {
        ...
        ord_shoptree.load("index.php?ajax=1&act=ord_shop_get&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),function(){
                has_shop_restrictions = ord_shoptree.getUserData(0, "has_shop_restrictions");
    ...
    ord_shoptree.attachEvent("onClick",function(idshop){
        ...
        if (idshop != shopselection)
        {
            ...
            {
                let allShops_item_found = ord_shoptree.getIndexById('all');
                if(allShops_item_found !== null) {
                    var children = ord_shoptree.getAllSubItems("all").split(",");
                } else {
                    var children = ord_shoptree.getAllChildless().split(",");
                }
            ...
            ord_filterPanel.setText('<?php echo _l('Filters', 1).' '._l('of', 1); ?> '+ord_shoptree.getItemText(shopselection));
...
?>
    ...
    var ps_order_states = [
    <?php
        $order_states = SCI::getScDisplayableOrderStates($sc_agent->id_lang);
        $return = '';
    ...
    ord_tb.setIconset('awesome');
    ord_tb.addButton("help", 0, "", "fad fa-question-circle blue", "fad fa-question-circle blue");
    ord_tb.setItemToolTip('help','<?php echo _l('Help', 1); ?>');
    ord_tb.addButton("set_states", 0, "", "fa fa-cog", "fa fa-cog");
    ord_tb.setItemToolTip('set_states','<?php echo _l('Manage order statuses', 1); ?>');
    ord_tb.addButton("refresh", 0, "", "fa fa-sync green", "fa fa-sync green");
    ord_tb.setItemToolTip('refresh','<?php echo _l('Refresh tree', 1); ?>');
    ord_tb.addButtonSelect('filter_view',0,'<?php echo _l('Filters view'); ?>',filter_views,'fad fa-flag blue','fad fa-flag blue',false,true);

    ord_tb.attachEvent("onClick", function(id){
        if (id=='help'){
            <?php echo "window.open('".getScExternalLink('support_orders_filter')."');"; ?>
        }
        if (id=='refresh'){
            displayFilters();
        }
        ...
        if(id=='all') {
            ...
            $.cookie('sc_ord_statusselection',statusselection, { expires: 60,path: cookiePath });
            $.cookie('sc_ord_periodselection','all', { expires: 60,path: cookiePath });
            displayOrders();
        }
        ...
        if (id=='btn_save'){
            ...
            if(filter_view_encoded.length === 0) {
                dhtmlx.message({text:'<?php echo _l('Pick a filter first', 1); ?>',type:'error',expire:3000});
                return false;
            }
                ...
                dhtmlx.confirm("<?php echo _l('You will delete the filter', 1); ?>: "+filter_used, function(result)
                {
                    if(result)
                    {
                        $.post("index.php?ajax=1&act=ord_tree_custom_update", {'action':'delete', 'filter_used': filter_used}, function (data) {
                            if(data !== 'KO') {
                                ord_tb.removeListOption('filter_view', filter_used);
        ...
        if(id=='set_states') {
            dhxMenu.callEvent('onClick',['ord_states']);
        }
        ...
        if (filter_views_list[id]!= null && filter_views_list[id]!= undefined && filter_views_list[id]!= ''){
            ...
            $.cookie('sc_ord_statusselection',statusselection, { expires: 60,path: cookiePath });
            $.cookie('sc_ord_periodselection',periodselection, { expires: 60,path: cookiePath });
            displayFilters('displayOrders()');
        }
    });
...
?>
    ...
    ord_filter.setImagePath('lib/js/imgs/dhxtree_material/');
...
?>
    ...
    ord_filter.attachEvent("onCheck",function(idfilter, state){
            ...
            if(idfilter == 'from_to')
            {
                ...
                wOrdFilterFromTo = dhxWins.createWindow("wOrdFilterFromTo", 170, 150, period_window_width, period_window_height);
                ...
                $.get("index.php?ajax=1&act=ord_filter_dates&id_lang="+SC_ID_LANG,function(data){
                        $('#jsExecute').html(data);
                    });
            ...
            if(idfilter == 'inv_from_to')
            {
                ...
                wOrdFilterFromTo.denyResize();
                wOrdFilterFromTo.setText('<?php echo _l('Select the date interval to filter', 1); ?>');
                $.get("index.php?ajax=1&act=ord_filter_dates&inv=1&id_lang="+SC_ID_LANG,function(data){
                        $('#jsExecute').html(data);
                    });
    ...
    ord_filter.attachEvent("onClick",function(idfilter){
        ...
        else
        {
            if(id_selected_segment === idfilter){
                ord_filter.clearSelection(idfilter);
...
?>
    ...
    function displayFilters(callback)
    {
        ...
        ord_filter.load("index.php?ajax=1&act=ord_filter_get&id_lang="+SC_ID_LANG+"&id_shop="+shopselection+"&"+new Date().getTime(),function(){
