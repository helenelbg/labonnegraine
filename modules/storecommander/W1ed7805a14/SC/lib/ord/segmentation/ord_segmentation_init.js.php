...
if (SCSG)
{
    ...
    prop_tb.setItemToolTip('segment_filter','<?php echo _l('Display only segments used by selected order', 1); ?>');
    ...
    prop_tb.setItemToolTip('segment_multi_add','<?php echo _l('Place selected order in selected segments', 1); ?>');
    prop_tb.addButton('segment_multi_del',1000,'','fad fa-unlink red','fad fa-unlink red');
    prop_tb.setItemToolTip('segment_multi_del','<?php echo _l('Remove selected order from selected segments', 1); ?>');
    ...
    function setPropertiesPanel_segments(id){
            ...
            prop_tb._segmentsGrid.attachEvent("onEditCell",function(stage,rId,cInd,nValue,oValue)
            {
                    ...
                    if (cInd == idxUsed){
                        if(stage==1)
                        {
                            ...
                            $.post("index.php?ajax=1&act=ord_segmentation_update&type=order&id_segment="+rId+"&action=present&value="+value+"&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),{ids:ord_grid.getSelectedRowId()},function(data){
                                displaySegments();
                            });
                        }
                    }
        ...
        {
            if(prop_tb._segmentsGrid.getSelectedRowId()!=null && lastOrderSelID!=null)
            {
                var value = 1;
                var segments = prop_tb._segmentsGrid.getSelectedRowId();
                $.post("index.php?ajax=1&act=ord_segmentation_update&type=order&segments="+segments+"&action=mass_present&value="+value+"&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),{ids:ord_grid.getSelectedRowId()},function(data){
                    displaySegments();
                });
            }
        }
    ...
    function displaySegments(force_refresh, callback)
    {
        ...
        {
            $.post("index.php?ajax=1&act=ord_segmentation_relation_get&type=order&idlist="+lastOrderSelID+"&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),function(data){
                selArray=data.split(',');
        ...
        {
            if ((ord_grid.getSelectedRowId()==null || ord_grid.getSelectedRowId()=='') && force_refresh!=true) return false;
            prop_tb._segmentsGrid.clearAll(true);
            prop_tb._segmentsGrid.load("index.php?ajax=1&act=ord_segmentation_get&type=orders&id_lang="+SC_ID_LANG,function()
            {
    ...
    let ord_segments_current_id = 0;
    ord_grid.attachEvent("onRowSelect",function (idproduct){
        if (propertiesPanel=='segments' && (ord_grid.getSelectedRowId()!==null && ord_segments_current_id!=idproduct)){
            if (ord_grid.getSelectedRowId()!=null)
            {
                displaySegments(true);
            }
            ord_segments_current_id=idproduct;
        }
    });
