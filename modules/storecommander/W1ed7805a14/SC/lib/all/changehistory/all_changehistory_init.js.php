<?php echo '<script type="text/javascript">'; ?>
    ...
    his_grid.setHeader("ID,<?php echo _l('ID employee'); ?>,<?php echo _l('Section'); ?>,<?php echo _l('Action'); ?>,<?php echo _l('Object'); ?>,<?php echo _l('Old value'); ?>,<?php echo _l('New value'); ?>,<?php echo _l('Object ID'); ?>,<?php echo _l('Lang ID'); ?>,<?php echo _l('Table'); ?>,<?php echo _l('Date'); ?><?php if (SCMS)
{
    echo ','._l('Shops');
} ?>");
    his_grid.setColumnIds("id_history,id_employee,section,action,object,oldvalue,newvalue,object_id,lang_id,table,date_add<?php if (SCMS)
{
    echo ',shops';
} ?>");
    his_grid.setInitWidths("50,50,90,75,100,100,100,60,60,100,115<?php if (SCMS)
{
    echo ',100';
} ?>");
    his_grid.setColAlign("right,left,left,left,left,left,left,right,right,left,left<?php if (SCMS)
{
    echo ',left';
} ?>");
    his_grid.setColTypes("ro,ro,ro,ro,ro,ed,ed,ro,ro,ro,ro<?php if (SCMS)
{
    echo ',ro';
} ?>");
    his_grid.setColSorting("int,int,str,str,str,str,str,int,int,str,str<?php if (SCMS)
{
    echo ',str';
} ?>");
    his_grid.attachHeader("#numeric_filter,#numeric_filter,#select_filter,#select_filter,#select_filter,#text_filter,#text_filter,#numeric_filter,#numeric_filter,#select_filter,#text_filter<?php if (SCMS)
{
    echo ',#text_filter';
} ?>");
    ...
    his_tb.setIconset('awesome');
    his_tb.addButton("delete", 0, "", "fa fa-minus-circle red", "fa fa-minus-circle red");
    his_tb.setItemToolTip('delete','<?php echo _l('Delete all history', 1); ?>');
    his_tb.addButton("refresh", 0, "", "fa fa-sync green", "fa fa-sync green");
    his_tb.setItemToolTip('refresh','<?php echo _l('Refresh grid', 1); ?>');
    his_tb.addButton("exportcsv", 100, "", "fad fa-file-csv green", "fad fa-file-csv green");
    his_tb.setItemToolTip('exportcsv','<?php echo _l('Export grid to clipboard in CSV format for MSExcel with tab delimiter.'); ?>');
    his_tb.addButton('history_rollback',100,'','fad fa-undo grey','fad fa-undo grey');
    ...
    his_tb.attachEvent("onClick",
        function(id)
        {
            ...
            else if (id == 'history_rollback')
            {
                ...
                displayQuickExportWindow(his_grid,1);
            }
        });
