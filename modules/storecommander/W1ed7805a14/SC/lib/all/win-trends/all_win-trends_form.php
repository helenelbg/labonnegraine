...
if (isset($_POST['saveTrendsActive']))
{
    ...
    if (version_compare(_PS_VERSION_, '1.5.0.0', '>='))
    {
        ...
        foreach ($shops as $shop)
        {
            ...
            if ($value == '1')
            {
                $return = sc_file_get_contents('http://api.storecommander.com/Trends/RegisterShop', 'POST', array(), $headers);
            ...
            {
                $return = sc_file_get_contents('http://api.storecommander.com/Trends/UnRegisterShop', 'POST', array(), $headers);
            }
        }
    }
    else
...
<input type="checkbox" name="trends_active" id="trends_active" onchange="$('#saveTrendsActive').click();" value="1" <?php if ($trends_active == '1')
{
    echo 'checked';
} ?> /> <label for="trends_active" style="cursor: pointer;"><?php echo _l('Yes I want to participate in E-commerce Trends project and I agree to send statistics'); ?></label>
