...
$supplier_list = Tools::getValue('supplier_list', null);
?>
<body style="margin:0;">
...
<script>
    ...
    let vaultObject = new dhx.Vault("file_uploader", {
        uploader:{
            target: 'index.php?ajax=1&act=all_upload&obj=supplier_logo&supplier_list=<?php echo $supplier_list; ?>&id_lang=<?php echo $id_lang; ?>',
            autosend:autosend
        },
        mode:"grid",
    ...
    vaultObject.events.on("UploadBegin", function(files){
        top.prop_tb._imagesUploadWindow['<?php echo $supplier_list; ?>'].park();
    });
    vaultObject.events.on("UploadComplete", function(files){
        ...
        if(error === 0) {
            top.prop_tb._imagesUploadWindow['<?php echo $supplier_list; ?>'].hide();
            top.displayImages();
            top.displaySuppliers();
        }
    });
</script>
</body>
