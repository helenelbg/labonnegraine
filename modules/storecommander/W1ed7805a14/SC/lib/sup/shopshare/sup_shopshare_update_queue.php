...
// FUNCTIONS
function removeInShop($supplier_id, $shop_id)
{
    if (!empty($supplier_id) && !empty($shop_id))
    {
        $sql = 'DELETE FROM `'._DB_PREFIX_.'supplier_shop` WHERE `id_supplier` = '.(int) $supplier_id.' AND id_shop = '.(int) $shop_id;
                    ...
                    if ($action_upd != '' && !empty($id_shop) && !empty($idlist))
                    {
                        switch ($action_upd) {
                            case 'present':
                                foreach ($ids as $id)
                                {
                                    $supplier = new Supplier($id, false, null, $id_actual_shop);

                                    if (!$supplier->isAssociatedToShop($id_shop) && $value == '1')
                                    {
                                        $supplier->id_shop_list = array($id_shop);
                                        $supplier->save();
                                    }
                                    elseif ($supplier->isAssociatedToShop($id_shop) && empty($value))
                                    {
                                ...
                                // pour un ou plusieurs suppliers passés en params
                            case 'mass_present':
