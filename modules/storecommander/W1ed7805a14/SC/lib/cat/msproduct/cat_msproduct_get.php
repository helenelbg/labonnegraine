<?php
    ...
    if(version_compare(_PS_VERSION_, '1.6.0.10', '>=')){
        $sql .=  ' WHERE 1'.(_s('CAT_PROD_TAX_ACTIVE') ? ' AND trg.active = 1 AND trg.deleted = 0' : '');
    } else {
    ...
    */
    ...
    function generateValue($col, $prodrow)
    {
        ...
        switch ($col){
            case 'wholesale_price':
                ...
                break;
            case 'quantity':
                $qty = SCI::getProductQty((int) $prodrow['id'], false, false, (int) $prodrow['id_selected_shop']);
                $return .= '<cell>'.$qty.'</cell>';
                break;
            ...
            case 'ecotax':
                ...
                break;
            case 'price':
                $return .= '<cell'.(sc_array_key_exists('wholesale_price', $prodrow) && $prodrow['wholesale_price'] > $prodrow['price'] ? ' bgColor="#FF0000"  style="color:#FFFFFF"' : '').'>'.number_format($prodrow['price'], 6, '.', '').'</cell>';
                break;
            ...
            case 'link_rewrite':
                ...
                break;
            case 'price_inc_tax':
                $return .= '<cell>'.number_format($prodrow['price'] * ($tax[(int) $prodrow['id_tax_rules_group']] / 100 + 1) + ($prodrow['ecotax'] * SCI::getEcotaxTaxRate()), 6, '.', '').'</cell>';
                break;
            case 'margin':
                $return .= '<cell></cell>';
                break;
     ...
     * PRODUCT SHOP
     */
    $sql = 'SELECT ps.* '.((version_compare(_PS_VERSION_, '1.5.0.0', '>=') ? ',(SELECT COUNT(id_product_attribute) FROM '._DB_PREFIX_.'product_attribute_shop WHERE id_product = ps.id_product AND id_shop = ps.id_shop) as nb_combination ' : ''));
    ...
    $sql .= ' WHERE ps.id_product IN ('.pInSQL($idlist).')
    ...
    foreach ($res as $product_by_shop)
    {
        if (!empty($product_by_shop['id_product']) && !empty($product_by_shop['id_shop']))
        {
            ...
            $product = array_merge($product, $product_by_shop);
            sc_ext::readCustomMsProductGridConfigXML('rowUserData', $product);
