<?php if (SCMS) {?>
    ...
    <?php if(version_compare(_PS_VERSION_, '1.7.2.0', '>=')){ ?>
        prop_tb.addButton("msproduct_refresh_physical_stocks",1000, "", "fad fa-retweet-alt green", "fad fa-retweet-alt green");
        prop_tb.setItemToolTip('msproduct_refresh_physical_stocks','<?php echo _l('Refresh stocks'); ?>');
    <?php } ?>
    ...
    getTbSettingsButton(prop_tb, {'grideditor':1,'settings':0}, 'prop_msproduct_',1000);
    ...
    function setPropertiesPanel_msproduct(id){
        if (id=='msproduct')
        {
            ...
            <?php if(version_compare(_PS_VERSION_, '1.7.2.0', '>=')){ ?>
                prop_tb.showItem('msproduct_refresh_physical_stocks');
            <?php } ?>
        ...
        if(id=='msproduct_refresh_physical_stocks'){
