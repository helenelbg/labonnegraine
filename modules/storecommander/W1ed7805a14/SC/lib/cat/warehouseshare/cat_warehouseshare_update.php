        ...
        case 'location':
            foreach ($ids as $id)
            {
                if (!empty($value))
                {
                    ...
                    if ($CAT_ADVANCEDSTOCKS_WAREHOUSESHARE_DEFAULT_TYPE == 1 && !$advanced_stock_management)
                    { // enabled
                        $shops = SCI::getSelectedShopActionList(false, (int) $id);
                        foreach ($shops as $shop)
                        {
                            StockAvailable::setProductDependsOnStock((int) $id, true, $shop);
                    ...
                    elseif ($CAT_ADVANCEDSTOCKS_WAREHOUSESHARE_DEFAULT_TYPE == 2 && !$advanced_stock_management)
                    { // enabled + manual
                        $shops = SCI::getSelectedShopActionList(false, (int) $id);
                        foreach ($shops as $shop)
                        {
                            StockAvailable::setProductDependsOnStock((int) $id, false, $shop);
