    ...
    switch ($action) {
        ...
        case 'active':
            foreach ($ids as $id)
            {
                ...
                {
                    $sql = 'UPDATE `'._DB_PREFIX_."product_shop` SET active='".(int) $value."' WHERE `id_product` = '".psql($id)."' AND id_shop = ".(int) $id_shop;
                    Db::getInstance()->Execute($sql);
                }
            }
        break;
        ...
        case 'present':
            foreach ($ids as $id)
            {
                ...
                if (!$product->isAssociatedToShop($id_shop) && $value == '1')
                {
                    ...
                    $product->save();
                    SCI::setQuantity($id, null, StockAvailable::getQuantityAvailableByProduct((int) $id, null, (int) $id_actual_shop), (int) $id_shop);
        ...
        case 'mass_active':
            ...
            foreach ($shops as $shop)
            {
                foreach ($ids as $id)
                {
                    ...
                    {
                        $sql = 'UPDATE `'._DB_PREFIX_."product_shop` SET active='".(int) $value."' WHERE `id_product` = '".psql($id)."' AND id_shop = ".(int) $shop;
                        Db::getInstance()->Execute($sql);
                    }
                }
            }
