    ...
    // Data update
     cat_treegrid_grid.attachEvent("onEditCell", function(stage,rId,cInd,nValue,oValue){
        ...
        if(stage==2)
        {
            ...
            $.post("index.php?ajax=1&act=cat_win-catmanagement_update&action=update&gr_id="+rId+"&field="+field+"&value="+nValue+'&id_shop='+id_shop+'&in_all_shops='+in_all_shops+'&id_lang='+id_actual_lang, function(data){
...
// PROPERTIES
    ...
    foreach ($sub_files as $sub_item)
    {
        ...
        {
            if (is_dir(SC_DIR.'lib/cat/win-catmanagement/'.$sub_item) && file_exists(SC_DIR.'lib/cat/win-catmanagement/'.$sub_item.'/cat_win-catmanagement_'.$sub_item.'_init.js.php'))
            {
                require_once SC_DIR.'lib/cat/win-catmanagement/'.$sub_item.'/cat_win-catmanagement_'.$sub_item.'_init.js.php';
            }
            elseif (is_dir(SC_DIR.'lib/cat/win-catmanagement/'.$sub_item) && file_exists(SC_DIR.'lib/cat/win-catmanagement/'.$sub_item.'/cat_win-catmanagement_'.$sub_item.'_init.php'))
            {
