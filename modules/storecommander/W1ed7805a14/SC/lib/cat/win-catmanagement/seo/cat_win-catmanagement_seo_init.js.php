...
cat_prop_tb.addListOption('cat_prop_subproperties', 'cat_prop_seo', 6, "button", '<?php echo _l('SEO', 1); ?>', "fad fa-at");
...
cat_prop_tb.attachEvent("onClick", function(id){
    if(id=="cat_prop_seo")
    {
        ...
        cat_prop_tb.setItemImage('cat_prop_subproperties', 'fad fa-at');
...
cat_prop_tb.addButton('cat_prop_seo_refresh',100,'','fa fa-sync green','fa fa-sync green');
cat_prop_tb.setItemToolTip('cat_prop_seo_refresh','<?php echo _l('Refresh grid', 1); ?>');
if (lightNavigation)
{
    cat_prop_tb.addButtonTwoState('cat_prop_seo_lightNavigation', 100, "", "fa fa-mouse-pointer", "fa fa-mouse-pointer");
    cat_prop_tb.setItemToolTip('cat_prop_seo_lightNavigation','<?php echo _l('Light navigation (simple click on grid)', 1); ?>');
}
cat_prop_tb.addButton("cat_prop_seo_exportcsv", 100, "", "fad fa-file-csv green", "fad fa-file-csv green");
cat_prop_tb.setItemToolTip('cat_prop_seo_exportcsv', '<?php echo _l('Export grid to clipboard in CSV format for MSExcel with tab delimiter.'); ?>');
cat_prop_tb.addButton('cat_prop_seo_selectall',100,'','fa fa-bolt yellow','fa fa-bolt yellow');
...
cat_prop_tb.attachEvent("onClick", function(id){
    ...
    if(id=='cat_prop_seo_exportcsv'){
        displayQuickExportWindow(cat_prop_seo_grid, 1);
    }
...
function initCatManagementPropSeo()
{
    ...
    cat_prop_tb.showItem('cat_prop_seo_exportcsv');
    ...
    // GRID
        ...
        cat_prop_seo_DataProcessor.setTransactionMode("POST");
            ...
            if(stage==2 && nValue!=oValue)
            {
                ...
                if (nValue!="" && cInd==idxLinkRewrite)
                {
                    ...
                    { ?>
                        ...
                        cat_prop_seo_grid.cells(rId,idxLinkRewrite).setValue(getLinkRewriteFromString(nValue.substr(0,<?php echo _s('CAT_LINK_REWRITE_SIZE'); ?>),id_lang));
                    <?php } ?>
                }
            }
            return true;
