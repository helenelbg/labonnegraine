        ...
        case 'mapping_process':
            ...
            switch ($importConfig[$filename]['idby']){
                case 'catname':
                    ...
                    {
                        exit(_l('Wrong mapping, mapping should contain the category name in').' '.$defaultLanguage->iso_code.'<br/><br/>'.
                ...
                case 'idcategory':
                    ...
                    {
                        exit(_l('Wrong mapping, mapping should contain the field: id_category'));
                    }
                    break;
                case 'path':
                    if (!sc_in_array('path', $mappingData['DBArray'], 'catWinCatImportProcess_DBArray'))
                    {
                        exit(_l('Wrong mapping, mapping should contain the field: path'));
                    }
                    break;
            }
            ...
            if ($importConfig[$TODOfilename]['fornewcat'] == 'create')
            {
                ...
                {
                    exit(_l('Wrong mapping, mapping should contain the fields: name + parents or path'));
                }
            }
            ...
            for ($current_line = $FIRST_CONTENT_LINE; ((($current_line <= (count($DATA) - 1)) && $line = parseCSVLine($importConfig[$TODOfilename]['fieldsep'], $DATA[$current_line])) && ($current_line <= $importlimit)); ++$current_line)
            {
                ...
                switch ($importConfig[$TODOfilename]['idby']) {
                    case 'catname':
                        ...
                        if (!empty($name))
                        {
                            $sql = 'SELECT c.id_category,c.date_upd
                                    FROM '._DB_PREFIX_.'category c
                                        LEFT JOIN '._DB_PREFIX_.'category_lang cl on (c.id_category=cl.id_category AND cl.id_lang='.(int) $defaultLanguage->id.")
                    ...
                    case 'idcategory':
                        ...
                        if (!empty($id_category_find))
                        {
                            $sql = 'SELECT c.id_category,c.date_upd
                                    FROM '._DB_PREFIX_."category c
                                    WHERE c.id_category=" .(int) $id_category_find . "
                                        ".(!empty($where_shop_list) ? ' AND EXISTS (SELECT * FROM '._DB_PREFIX_.'category_shop cs WHERE c.id_category=cs.id_category AND id_shop IN ('.psql($where_shop_list).'))' : '').'
                                    LIMIT 1';
                        }
                        else
                    ...
                    case 'path':
                        ...
                        { // category path
                            $id_category = (sc_array_key_exists(($c), $categoryIDByPath) ? (int) $categoryIDByPath[($c)] : 0);
                        }
                        break;
                ...
                if ($scdebug)
                {
                    echo findCSVLineValue('reference').' : '.$id_category.'<br/>';
                }
                if ($scdebug)
                {
                    echo 'a';
                ...
                elseif (empty($id_category))
                {
                    ...
                    elseif ($importConfig[$TODOfilename]['fornewcat'] == 'create' || $importConfig[$TODOfilename]['fornewcat'] == 'createall')
                    {
                        ...
                        if (fieldInMapping('path'))
                        {
                            ...
                            else
                            {
                                ...
                                if (strpos($c, '>') !== false)
                                {
                                    ...
                                    if ((strpos($c, '>') !== false) || (in_array(trim($c), $categoriesFirstLevel)))
                                    { // category path
                                        $idc = (sc_array_key_exists(($c), $categoryIDByPath) ? (int) $categoryIDByPath[($c)] : 0);
                                    }
                                    if ($idc != 0)
                                    { // IF PARENTS EXIST
                                        $id_parent = (int) $idc;
                        ...
                        $newcat->link_rewrite[$defaultLanguage->id] = link_rewrite($name_cat, $defaultLanguage->iso_code);
                        foreach ($languages as $lang)
                        {
                            $newcat->name[$lang['id_lang']] = trim($name_cat);
                            $newcat->link_rewrite[$lang['id_lang']] = link_rewrite($name_cat, $lang['iso_code']);
                ...
                foreach ($line as $key => $value)
                {
                    ...
                    if (sc_array_key_exists($key, $firstLineData) && sc_in_array($firstLineData[$key], $mappingData['CSVArray'], 'catWinCatImportProcess_CSVArray'))
                    {
                        ...
                        switch ($switchObject) {
                            case 'active':$newcat->active = (int) getBoolean($value); break;
                            case 'position':$newcat->position = (int) $value; break;
                            case 'id_parent':
                                if (Category::categoryExists((int) $value))
                                {
                                    $newcat->id_parent = (int) $value;
                                }
                                break;
                            case 'link_rewrite': if ($value != '')
                            {
                                $newcat->link_rewrite[$id_lang] = link_rewrite($value, $iso_lang);
                            }break;
                            case 'name':
                ...
                if ($newcat->save())
                {
                    $categories[trim($newcat->name[$defaultLanguage->id])] = array('id_category' => $newcat->id, 'id_parent' => $newcat->id_parent);
                    $categoryNameByID[$newcat->id] = $newcat->name[$defaultLanguage->id];
                    ...
                    {
                        $categoriesFirstLevel[] = $newcat->name[$defaultLanguage->id];
                    }
                    ...
                    $TODO[] = 'UPDATE '._DB_PREFIX_."category SET `date_upd`='".psql(date('Y-m-d H:i:s'))."' WHERE id_category=".(int) $newcat->id;
                    foreach ($TODO as $sql)
                    {
                            ...
                            switch ($switchObject) {
                                ...
                                case 'customergroups':
                                    if ($value != '' && $value != '0' && $newcat->id)
                                    {
                                        ...
                                        {
                                            $id_group = (sc_array_key_exists($group, $dataDB_customergroupByName) ? (int) $dataDB_customergroupByName[$group] : 0);
                                ...
                                case 'thumbnailURL':
                                    if ($value != '')
                                    {
                                        ...
                                        $image_name = _PS_CAT_IMG_DIR_.(int) $newcat->id.'_thumb.jpg';
                                        @unlink($image_name);
                                        if (copy($imagefilename, $image_name))
                                        {
                                            $images_types = ImageType::getImagesTypes('categories');
                                            ...
                                            foreach ($images_types as $k => $image_type)
                                            {
                                                if (version_compare(_PS_VERSION_, '1.7.0.0', '>='))
                                                {
                                                    if ($formatted_small == $image_type['name'])
                                                    {
                                                        ...
                                                        {
                                                            $infos = getimagesize($imagefilename);
                                                            if (!empty($infos) && is_array($infos))
                                                            {
                                                                ImageManager::resize(
                                                                    $imagefilename,
                                                                    _PS_CAT_IMG_DIR_.$newcat->id.'_thumb.jpg',
                                                                    (int) $infos[0],
                                                                    (int) $infos[1]
                                                                );
                                                            }
