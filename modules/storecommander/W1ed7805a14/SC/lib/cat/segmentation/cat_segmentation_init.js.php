...
if (SCSG)
{
    ...
    prop_tb.setItemToolTip('segment_expand','<?php echo _l('Expand all segments', 1); ?>');
    prop_tb.addButton('segment_collapse',1000,'','fa fa-compress-arrows-alt green','fa fa-compress-arrows-alt green');
    prop_tb.setItemToolTip('segment_collapse','<?php echo _l('Collapse all segments', 1); ?>');
    ...
    function displaySegments(force_refresh, callback)
    {
        ...
        else if(force_refresh==true)
        {
            ...
            prop_tb._segmentsGrid.load("index.php?ajax=1&act=cat_segmentation_get&type=catalog&id_lang="+SC_ID_LANG,function()
            {
    ...
    let segments_current_id = 0;
    cat_grid.attachEvent("onRowSelect",function (idproduct){
        if (propertiesPanel=='segments' && (cat_grid.getSelectedRowId()!==null && segments_current_id!=idproduct)) {
            displaySegments(true);
            segments_current_id=idproduct;
        }
    });
