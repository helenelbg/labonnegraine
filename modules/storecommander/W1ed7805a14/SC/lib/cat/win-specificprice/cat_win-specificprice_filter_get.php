...
$icon = 'catalog.png';
     ...
     * Date
     */
    echo '<item id="Gdat" text="'._l('Active specific prices').'" im0="'.$icon.'" im1="'.$icon.'" im2="'.$icon.'" open="1">'."\n";
        ...
        echo '<item id="dat_futur" text="'._l('After').' '.$dateT.' '._l('(not unlimited)').'" im0="'.$icon.'" im1="'.$icon.'" im2="'.$icon.'" '.(sc_in_array('dat_futur', $selection, 'catWinSpecPriceFilterGet_selection') ? 'checked="1"' : '').'></item>'."\n";
    ...
    */
    ...
    foreach ($manufacturers as $manufacturer)
    {
        ...
        $name = str_replace('>', '2', $name);
        $name = str_replace('"', "''", $name);
        echo '<item id="man_'.$manufacturer['id_manufacturer'].'" text="'.$name.'" im0="'.$icon.'" im1="'.$icon.'" im2="'.$icon.'" '.(sc_in_array('man_'.$manufacturer['id_manufacturer'], $selection, 'catWinSpecPriceFilterGet_selection') ? 'checked="1"' : '').'>'."\n";
        echo '</item>'."\n";
    }
    echo '</item>'."\n";
