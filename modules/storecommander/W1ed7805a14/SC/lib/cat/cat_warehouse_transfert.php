...
if (!empty($id_warehouse_A) && !empty($id_warehouse_B) && $id_warehouse_A != $id_warehouse_B)
{
    ...
    AND    ( 
            (id_product_attribute = 0 AND id_product NOT IN (SELECT id_product FROM `'._DB_PREFIX_.'warehouse_product_location` WHERE id_warehouse = "'.(int) $id_warehouse_B.'" AND id_product_attribute = 0))
            OR
            ( id_product_attribute>0 AND id_product_attribute NOT IN (SELECT id_product_attribute FROM `'._DB_PREFIX_.'warehouse_product_location` WHERE id_warehouse = "'.(int) $id_warehouse_B.'") )
        )';
