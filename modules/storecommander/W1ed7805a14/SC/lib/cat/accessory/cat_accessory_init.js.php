 ...
 if (_r('GRI_CAT_PROPERTIES_GRID_ACCESSORIES')) { ?>
    ...
    prop_tb.attachEvent("onStateChange",function(id,state){
        if (id=='accessory_loadallproducts'){
            ...
            $.cookie('sc_cat_accessory_display_all_products',display_all_products, { expires: 3600 , path: cookiePath});
...
function displayAccessories(callback,forceAllProducts)
{
    if (prop_tb._accessoriesGrid._rowsNum > 0 && accessoriesFilterByProducts == 0 && productsFilterByCategory == 0)
    {
        ...
        prop_tb._accessoriesGrid.clearAll(true);
        prop_tb._accessoriesGrid.load("index.php?ajax=1&act=cat_accessory_get&filter_params="+filter_params+(cat_grid.getSelectedRowId()!=null?"&idlist="+cat_grid.getSelectedRowId():"")+(display_all_products==1 || productsFilterByCategory == 1?"&forceAllProducts=1":"")+"&accessory_filter="+accessoriesFilter+"&accessoriesFilterByProducts="+accessoriesFilterByProducts+"&id_category="+catselection+"&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),function(){
