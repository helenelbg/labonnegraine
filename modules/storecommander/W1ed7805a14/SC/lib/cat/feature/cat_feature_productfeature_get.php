<?php
    ...
    if ($filter)
    {
        ...
        foreach ($res as $val)
        {
            $featuresSelection[(int) $val['id_feature']] = (int) $val['id_feature'];
        }
    }
    ...
    LEFT JOIN `'._DB_PREFIX_.'feature_lang` fl ON (fv.`id_feature` = fl.`id_feature` AND fl.`id_lang` = '.(int) $id_lang.')
