...
function getCats()
{
    ...
    // Category lang
    $categLangSql = 'SELECT *'.(version_compare(_PS_VERSION_, '1.5.1.0', '<') ? ', 0 as id_shop' : '').'
                FROM '._DB_PREFIX_.'category_lang
                GROUP BY id_category'.(SCMS ? ', id_shop' : '').', id_lang';
    $categLangRes = Db::getInstance()->ExecuteS($categLangSql);
    if (!empty($categLangRes))
    {
    ...
    if (!empty($res))
    {
        foreach ($res as $categ)
        {
            ...
            $categoryNameByID[$categ['id_category']][$categ['id_shop']] = $categ['name'];
        ...
        foreach ($res as $categ)
        {
            ...
            $groups = '';
            if (!empty($groupsArray) && array_key_exists($categ['id_category'], $groupsArray))
            {
