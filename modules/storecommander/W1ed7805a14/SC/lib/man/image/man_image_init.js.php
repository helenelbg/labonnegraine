...
?>
                ...
                if (!dhxWins.isWindow("wProductImages"+manudacturers_ids))
                {
                    ...
                    ll_toolbar.attachEvent("onStateChange", function(id,state){
                            if (id=='auto_upload'){
                                ...
                                $.cookie('sc_man_img_auto_upload',auto_upload, { expires: 60,path: cookiePath });
                            }
                        });
        ...
        if (id=='image_del')
        {
            ...
            {
                $.post("index.php?ajax=1&act=man_image_update&action=delete&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),{
                    manufacturer_list: prop_tb._imagesGrid.getSelectedRowId()
                },function(){
                    displayImages();
...
function displayImages(callback)
{
    ...
    prop_tb._imagesGrid.load("index.php?ajax=1&act=man_image_get&id_manufacturer="+man_grid.getSelectedRowId()+"&id_shop="+shopselection+"&id_lang="+SC_ID_LANG,function()
            {
