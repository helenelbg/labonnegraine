...
?>
    ...
    prop_tb.addButtonSelect('panel',1000,'<?php echo _l('Descriptions', 1); ?>',icons,'fa fa-search blue','fa fa-search blue',false,true);
...
<?php
    ...
    foreach ($files as $item)
    {
        if ($item != '.' && $item != '..')
        {
            if (is_dir(SC_TOOLS_DIR.'lib/man/'.$item) && file_exists(SC_TOOLS_DIR.'lib/man/'.$item.'/man_'.$item.'_init.js.php') && substr($item, 0, 4) != 'win-')
            {
                // OVERRIDE
                require_once SC_TOOLS_DIR.'lib/man/'.$item.'/man_'.$item.'_init.js.php';
                if ($item == 'combination')
                {
                    ...
                    foreach ($sub_files as $sub_item)
                    {
                        ...
                        {
                            if (is_dir(SC_TOOLS_DIR.'lib/man/'.$item.'/'.$sub_item) && file_exists(SC_TOOLS_DIR.'lib/man/'.$item.'/'.$sub_item.'/man_'.$item.'_'.$sub_item.'_init.js.php'))
                            {
                                require_once SC_TOOLS_DIR.'lib/man/'.$item.'/'.$sub_item.'/man_'.$item.'_'.$sub_item.'_init.js.php';
                            }
                        }
                    }
                }
            }
            elseif (is_dir(SC_DIR.'lib/man/'.$item) && file_exists(SC_DIR.'lib/man/'.$item.'/man_'.$item.'_init.js.php') && substr($item, 0, 4) != 'win-')
            {
                // STANDARD BEHAVIOR
                require_once SC_DIR.'lib/man/'.$item.'/man_'.$item.'_init.js.php';
                if ($item == 'combination')
                {
                    ...
                    foreach ($sub_files as $sub_item)
                    {
                        if ($sub_item != '.' && $sub_item != '..')
                        {
                            if (is_dir(SC_DIR.'lib/man/'.$item.'/'.$sub_item) && file_exists(SC_DIR.'lib/man/'.$item.'/'.$sub_item.'/man_'.$item.'_'.$sub_item.'_init.js.php'))
                            {
                                require_once SC_DIR.'lib/man/'.$item.'/'.$sub_item.'/man_'.$item.'_'.$sub_item.'_init.js.php';
                            }
                        }
                    }
                }
            }
            elseif (is_dir(SC_TOOLS_DIR.'lib/man/'.$item) && file_exists(SC_TOOLS_DIR.'lib/man/'.$item.'/man_'.$item.'_init.php') && substr($item, 0, 4) != 'win-')
            {
                // OVERRIDE
