        ...
        foreach ($rows as $num => $row)
        {
            if (!empty($log_ids[$num]))
            {
                ...
                if (!empty($action) && $action == 'update' && !empty($gr_id))
                {
                    ...
                    if (count($todo))
                    {
                        $sql = 'UPDATE '._DB_PREFIX_.'manufacturer_lang SET '.join(' , ', $todo)." WHERE id_manufacturer=".(int) $id_manufacturer." AND id_lang=".(int) $id_lang;
                        Db::getInstance()->Execute($sql);
                    }

                    //update date_upd
                    $sql = 'UPDATE '._DB_PREFIX_.'manufacturer SET '.(isset($_POST['name']) ? "name='".psql(html_entity_decode(Tools::getValue('name')))."'," : '')." date_upd = '".pSQL(date('Y-m-d H:i:s'))."' WHERE id_manufacturer=".(int) $id_manufacturer.';';
                    Db::getInstance()->Execute($sql);
                }
