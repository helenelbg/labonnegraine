<script type="text/javascript">
    ...
    man_grid_tb.setIconset('awesome');
    man_grid_tb.addButton("refresh", 100, "", "fa fa-sync green", "fa fa-sync green");
    man_grid_tb.setItemToolTip('refresh','<?php echo _l('Refresh grid'); ?>');
    man_grid_tb.addButton("add", 100, "", "fa fa-plus-circle green", "fa fa-plus-circle green");
    man_grid_tb.setItemToolTip('add','<?php echo _l('Create new manufacturer'); ?>');
    man_grid_tb.addButton("delete", 100, "", "fa fa-minus-circle red", "fa fa-minus-circle red");
    man_grid_tb.setItemToolTip('delete','<?php echo _l('This will permanently delete the selected manufacturer'); ?>');
    man_grid_tb.addButton("selectall", 100, "", "fa fa-bolt yellow", "fad fa-bolt grey");
    man_grid_tb.setItemToolTip('selectall','<?php echo _l('Select all manufacturers'); ?>');
    <?php if (_r('ACT_CAT_FAST_EXPORT')) { ?>
    man_grid_tb.addButton("exportcsv", 100, "", "fad fa-file-csv green", "fad fa-file-csv green");
    man_grid_tb.setItemToolTip('exportcsv','<?php echo _l('Export grid to clipboard in CSV format for MSExcel with tab delimiter.'); ?>');
    <?php } ?>
    man_grid_tb.addButton("help", 100, "", "fad fa-question-circle blue", "fad fa-question-circle blue");
    ...
    man_grid_tb.addButtonSelect('lang',0,'<?php echo $clang; ?>',opts,'fad fa-flag blue','fad fa-flag blue',false,true);
    ...
    man_grid_tb.addButtonSelect("gridview", 0, "<?php echo _l('Light view'); ?>", opts, "fad fa-ruler-triangle", "fad fa-ruler-triangle",false,true);
    ...
    function gridToolBarOnClick(id){
        ...
        if (id=='help'){
            <?php echo "window.open('".getScExternalLink('support_manufacturers')."');"; ?>
        }
        if (id=='filters_reset')
        {
        ...
        if (id=='exportcsv'){
            displayQuickExportWindow(man_grid,1);
        }
...
//            console.log(cInd);
    ...
    function onGridManufacturerContextButtonClick(itemId){
        ...
        if (itemId=="gopsbo"){
            ...
            wModifyManufacturer.attachURL("<?php echo SC_PS_PATH_ADMIN_REL; ?>index.php?<?php echo version_compare(_PS_VERSION_, '1.5.0.0', '>=') ? 'controller=AdminManufacturers' : 'tab=AdminManufacturers'; ?>&updatemanufacturer&id_manufacturer="+tabId+"&id_lang="+SC_ID_LANG+"&adminlang=1&token=<?php echo $sc_agent->getPSToken('AdminManufacturers'); ?>");
    ...
    function doOnRowSelected(idmanufacturer){
        if (!dhxLayout.cells('b').isCollapsed() && last_manufacturerID!=idmanufacturer)
        {
            ...
            idxName=man_grid.getColIndexById('name');
            let countSelection = man_grid.getSelectedRowId().split(',').length;
            let propTitle = '<?php echo _l('Properties', 1).' '._l('of', 1); ?> '+man_grid.cells(last_manufacturerID,idxName).getValue();
            if(countSelection>1) {
                propTitle = '<?php echo _l('Properties', 1).' '._l('for', 1); ?> '+ countSelection + ' <?php echo strtolower(_l('Manufacturers')); ?>';
            }
    ...
    function displayManufacturers(callback, firsttime = null)
    {
        if(firsttime!=undefined && firsttime!=null && firsttime!="") {
            ...
            echo 'shopselection = "'.implode(',',$shops).'";';
            ?>
        }
        if (shopselection!=undefined && shopselection!=null && shopselection!="")
        {
            ...
            let loadUrl = "index.php?ajax=1&act=man_manufacturer_get&tree_mode="+tree_mode+"&manufacturerfrom="+displayManufacturersFrom+"&idshop="+shopselection+"&view="+gridView+params_supp+"&id_lang="+SC_ID_LANG+"&"+new Date().getTime();
            ajaxPostCalling(man_manufacturerPanel, man_grid, loadUrl,function(data)
            {
                man_grid.parse(data);
