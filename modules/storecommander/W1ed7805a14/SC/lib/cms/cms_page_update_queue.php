...
if (Tools::getValue('rows') || $action == 'insert')
{
    ...
    if (is_array($rows) && count($rows) > 0)
    {
        ...
        foreach ($rows as $num => $row)
        {
            if (!empty($log_ids[$num]))
            {
                ...
                elseif (!empty($action) && $action == 'update' && !empty($gr_id))
                {
                    ...
                    $todo_lang = array();
                    $versSuffix = '';
                    sc_ext::readCustomCMSGridsConfigXML('onBeforeUpdateSQL');
                    ...
                    foreach ($fields_lang as $field)
                    {
                        if (isset($_POST[$field]))
                        {
                            ...
                            {
                                $todo_lang[] = "`link_rewrite`='".pSQL(link_rewrite($value, Language::getIsoById($id_lang)))."'";
                            }
                    ...
                    sc_ext::readCustomCMSGridsConfigXML('onAfterUpdateSQL');
                }
        ...
        sc_ext::readCustomCMSGridsConfigXML('extraVars');
