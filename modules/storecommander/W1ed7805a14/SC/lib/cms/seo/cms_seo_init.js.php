    prop_tb.addListOption('panel', 'cmsseo', 15, "button", '<?php echo _l('SEO', 1); ?>', "fad fa-at");
    ...
    function initCmsSeo()
    {
        if (needInitCmsSeo)
        {
            ...
            prop_tb._cmsSeo_tb = prop_tb._cmsSeo.attachToolbar();
             prop_tb._cmsSeo_tb.setIconset('awesome');
            prop_tb._cmsSeo_tb.addButton("CmsSeo_refresh", 100, "", "fa fa-sync green", "fa fa-sync green");
            prop_tb._cmsSeo_tb.setItemToolTip('CmsSeo_refresh','<?php echo _l('Refresh grid', 1); ?>');
            prop_tb._cmsSeo_tb.addButton("exportcsv", 100, "", "fad fa-file-csv green", "fad fa-file-csv green");
            ...
            prop_tb._cmsSeo_tb.attachEvent("onClick", function(id){
                    if (id=='CmsSeo_refresh')
                    {
                        ...
                        displayQuickExportWindow(prop_tb._cmsSeoGrid,1);
                    }
                });
    ...
    function onEditCellCmsSeo(stage,rId,cInd,nValue,oValue)
    {
        ...
        if (stage==2 && nValue!=oValue)
        {
            ...
            let idxLinkRewrite=prop_tb._cmsSeoGrid.getColIndexById('link_rewrite');
            if (nValue!="" && cInd==prop_tb._cmsSeoGrid.getColIndexById('meta_title'))
            {
                <?php $accented = Configuration::get('PS_ALLOW_ACCENTED_CHARS_URL');
                if ($accented == 1) {    ?>
                    params.params['link_rewrite'] =  getAccentedLinkRewriteFromString(nValue.substr(0,<?php echo _s('CMS_LINK_REWRITE_SIZE'); ?>));
                <?php }
                else
                { ?>
                    let rId_splitted = rId.split('_');
                    let id_lang = Number(rId_splitted[1]);
                    params.params['link_rewrite'] =  getLinkRewriteFromString(nValue.substr(0,<?php echo _s('CMS_LINK_REWRITE_SIZE'); ?>),id_lang);
                <?php } ?>
                prop_tb._cmsSeoGrid.cells(rId,idxLinkRewrite).setValue(params.params['link_rewrite']);
            }
    ...
    let cmsseo_current_id = 0;
    cms_grid.attachEvent("onRowSelect",function (idcms){
        if (propertiesPanel=='cmsseo' && (cms_grid.getSelectedRowId()!==null && cmsseo_current_id!=idcms)){
            //initCmsSeo();
            displayCmsSeo();
            displayGoogleAdwords();
            cmsseo_current_id=idcms;
        }
    });
