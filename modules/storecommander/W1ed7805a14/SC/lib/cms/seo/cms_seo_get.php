...
function getRowsFromDB()
{
    ...
    $sql = 'SELECT cl.*'.(version_compare(_PS_VERSION_, '1.5.0.0', '>=') ? ', cs.id_shop' : '').'
            FROM `'._DB_PREFIX_.'cms_lang` cl'
            .(!_s('CMS_PAGE_LANGUAGE_ALL') ? ' INNER JOIN `'._DB_PREFIX_.'lang` `l` ON cl.id_lang = l.id_lang AND l.active = 1' : '')
            .(version_compare(_PS_VERSION_, '1.5.0.0', '>=') ? ' INNER JOIN `'._DB_PREFIX_.'cms_shop` `cs` ON cs.id_cms = cl.id_cms AND cl.id_shop = '.(int) SCI::getSelectedShop() : '').
            ' WHERE (cl.id_cms IN ('.pInSQL($idlist).'))
            ORDER BY cl.id_cms, cl.id_lang'.(version_compare(_PS_VERSION_, '1.5.0.0', '>=') ? ', cs.id_shop' : '');
