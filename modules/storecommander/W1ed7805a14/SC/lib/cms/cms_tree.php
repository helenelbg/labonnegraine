<script type="text/javascript">
    ...
    cms_shoptree.setImagePath('lib/js/imgs/dhxtree_material/');
    ...
    var cmsShoptreeTB = cms_storePanel.attachToolbar();
      cmsShoptreeTB.setIconset('awesome');
    cmsShoptreeTB.addButton("help", 0, "", "fad fa-question-circle blue", "fad fa-question-circle blue");
    ...
    {
        let allShops_item_found = cms_shoptree.getIndexById('all');
        if(allShops_item_found !== null) {
            var children = cms_shoptree.getAllSubItems("all").split(",");
        } else {
            var children = cms_shoptree.getAllChildless().split(",");
        }
    ...
    function saveCheckSelection()
    {
        ...
        if(shopselection=="all" || shopselection=="0")
        {
            ...
            if(allShops_item_found !== null) {
                ...
                checked = cms_shoptree.getAllChildless();
            }
        }
    ...
    function displayShopTree(callback) {
        ...
        cms_shoptree.load("index.php?ajax=1&act=cms_shop_get&id_lang="+SC_ID_LANG+"&"+new Date().getTime(),function(){
                ...
                if (cmsselection!=0)
                {
                    ...
                    if (cname!=null)
                        $.post("index.php?ajax=1&act=cms_category_update&action=insert&id_parent="+cmsselection+'&id_lang='+SC_ID_LANG,{name: (cname)},function(id){
                            cms_tree.insertNewChild(cmsselection,id,cname,0,'folder_grey.png','folder_grey.png','folder_grey.png');
                        });
                }else{
    ...
    { ?>
                    ...
                    <?php } ?>
                    pushOneUsage('tree-bo-link-admincmscontent_addcms_category','cms');
    ...
    cms_tree.setImagePath('lib/js/imgs/dhxtree_material/');
    ...
    cms_tree.attachEvent("onDrag",function(sourceid,targetid,sibling,sourceobject,targetobject){
        ...
        if (sourceobject._name=='grid' && targetobject._name=='tree') return false;
        <?php } ?>
        if (targetid==0) {targetid=cms_tree.getUserData("","parent_root");}
        if (sourceobject._name=='grid')
        {
            ...
            if (dragdropcache!=cmsselection+'-'+targetid+'-'+cmsPages)
            {
                ...
                $.post("index.php?ajax=1&act=cms_category_droppageoncategory&id_lang="+SC_ID_LANG,{'categoryTarget':targetid,'categorySource':CMScategorySource,'cmsPages':cmsPages},function(){
                    if (draggedCmsPage>0)
                    {
                    ...
                    targetobject.setItemStyle(targetid,'background:none;');
    ...
    cms_tree.attachEvent("onBeforeContextMenu", function(itemId){
        ...
        cms_cmenu_tree.showItem('goshop');
         <?php if (version_compare(_PS_VERSION_, '1.7.0.0', '>=')) {?>
            if(Number(cms_tree.getUserData(itemId,"is_home")) === 1) {
                cms_cmenu_tree.hideItem('gopsbo');
            } else {
    ...
    function displayTree(callback)
    {
        ...
        cms_tree.load("index.php?ajax=1&act=cms_category_get&id_lang="+SC_ID_LANG+"&id_shop="+shopselection+"&"+new Date().getTime(),function(){
            if (cmsselection!=0) {
