...
// CATEGORIES TREE GRID
    ...
    var cms_treegrid_tb = dhxlCmsCatManagement.cells('a').attachToolbar();
      cms_treegrid_tb.setIconset('awesome');

    cms_treegrid_tb.addButton('cms_treegrid_refresh',100,'','fa fa-sync green','fa fa-sync green');
    cms_treegrid_tb.setItemToolTip('cms_treegrid_refresh','<?php echo _l('Refresh', 1); ?>');
    if (lightNavigation)
    {
        cms_treegrid_tb.addButtonTwoState('lightNavigation', 100, "", "fa fa-mouse-pointer", "fa fa-mouse-pointer");
    ...
    else
    {
        ...
        cms_treegrid_tb.setItemToolTip('cms_treegrid_shop','<?php echo _l('Shop'); ?>');
        cms_treegrid_tb.addButtonTwoState("cms_treegrid_inallshops", 100, '<?php echo _l('Update all shops'); ?>', "far fa-square", "far fa-check-square");
        ...
        in_all_shops=1;
        cms_treegrid_tb.setItemImage("cms_treegrid_inallshops", "far fa-check-square");
    ...
    <?php
        ...
        cms_treegrid_tb.addButtonSelect('cms_treegrid_lang',100,'<?php echo strtoupper($clang); ?>',opts,'fad fa-flag blue','fad fa-flag blue',false,true);
    ...
    cms_treegrid_tb.addButton("cms_treegrid_selectall", 100, "", "fa fa-bolt yellow", "fa fa-bolt yellow");
    cms_treegrid_tb.setItemToolTip('cms_treegrid_selectall','<?php echo _l('Select all', 1); ?>');
    cms_treegrid_tb.addButton("cms_treegrid_seeinsc", 100, "", "fad fa-search", "fad fa-search");
    cms_treegrid_tb.setItemToolTip('cms_treegrid_seeinsc','<?php echo _l('Open this category in SC', 1); ?>');
    <?php if (_r('ACT_CMS_ADD_CATEGORY')) { ?>
    cms_treegrid_tb.addButton("cms_treegrid_add", 100, "", "fa fa-plus-circle green", "fa fa-plus-circle green");
    cms_treegrid_tb.setItemToolTip('cms_treegrid_add','<?php echo _l('Create new category', 1); ?>');
    <?php } ?>
    cms_treegrid_tb.addButton("cms_treegrid_in_bin", 100, "", "fad fa-trash grey", "fad fa-trash grey");
    cms_treegrid_tb.setItemToolTip('cms_treegrid_in_bin','<?php echo _l('Put in the bin/trash', 1); ?>');
    <?php if (_r('ACT_CMS_EMPTY_RECYCLE_BIN')) { ?>
    cms_treegrid_tb.addButton("cms_treegrid_bin", 100, "", "fa fa-trash-alt red", "fa fa-trash-alt red");
    cms_treegrid_tb.setItemToolTip('cms_treegrid_bin','<?php echo _l('Empty bin', 1); ?>');
    <?php } ?>
    cms_treegrid_tb.addSeparator("sep003",100);
    cms_treegrid_tb.addButton("cms_treegrid_active_cms", 100, "", "fa fa-lightbulb-on yellow", "fa fa-lightbulb-on yellow");
    cms_treegrid_tb.setItemToolTip('cms_treegrid_active_cms','<?php echo _l('Activate all cms for the selected categories', 1); ?>');
    cms_treegrid_tb.addButton("cms_treegrid_desactive_cms", 100, "", "fad fa-lightbulb grey", "fad fa-lightbulb grey");
...
// PROPERTIES
    ...
    var cms_prop_tb = dhxlCmsCatManagement.cells('b').attachToolbar();
      cms_prop_tb.setIconset('awesome');

    actual_cmscatmanagement_subproperties = "cms_prop_info";

    var opts = new Array();
    cms_prop_tb.addButtonSelect("cms_prop_subproperties", 100, "<?php echo _l('Name & description'); ?>", opts, "fad fa-align-left", "fa fa-search blue",false,true);
    ...
    foreach ($sub_files as $sub_item)
    {
        ...
        {
            if (is_dir(SC_DIR.'lib/cms/win-cmscatmanagement/'.$sub_item) && file_exists(SC_DIR.'lib/cms/win-cmscatmanagement/'.$sub_item.'/cms_win-cmscatmanagement_'.$sub_item.'_init.js.php'))
            {
                require_once SC_DIR.'lib/cms/win-cmscatmanagement/'.$sub_item.'/cms_win-cmscatmanagement_'.$sub_item.'_init.js.php';
            }
            elseif (is_dir(SC_DIR.'lib/cms/win-cmscatmanagement/'.$sub_item) && file_exists(SC_DIR.'lib/cms/win-cmscatmanagement/'.$sub_item.'/cms_win-cmscatmanagement_'.$sub_item.'_init.php'))
            {
...
function displayTreegridCategories()
{
    ...
    cms_treegrid_grid.load("index.php?ajax=1&act=cms_win-cmscatmanagement_get&id_shop="+id_shop+"&id_lang="+id_actual_lang,function()
    {
//         UISettings
