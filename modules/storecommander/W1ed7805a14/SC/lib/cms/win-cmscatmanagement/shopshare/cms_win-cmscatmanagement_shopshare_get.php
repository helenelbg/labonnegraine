    ...
    function getRows()
    {
        ...
        if (!$multiple)
        {
            foreach ($res as $shop)
            {
                $sql2 = 'SELECT cs.id_cms_category, c.active
                        ...
                        WHERE c.id_cms_category IN ('.pInSQL($idList).")
        ...
        else
        {
            $sql3 = 'SELECT id_shop
                    ...
                    WHERE id_cms_category IN ('.pInSQL($idList).')';
            ...
            foreach ($res as $shop)
            {
                ...
                if ($nb_present == $cntCmsCategories)
                {
                    $used[$shop['id_shop']][0] = 1;
                    $used[$shop['id_shop']][2] = '7777AA';
                }
                elseif ($nb_present < $cntCmsCategories && $nb_present > 0)
                {
