<?php
    ...
    function getRowsFromDB()
    {
        ...
        WHERE cl.id_cms_category IN ('.pInSQL($idlist).')
        ...
        foreach ($res as $row)
        {
            $xml .= "<row id='".$row['id_cms_category'].'_'.$row['id_lang'].(SCMS ? '_'.$row['id_shop'] : '')."'>";
...
<head>
<beforeInit>
<call command="attachHeader"><param><![CDATA[#text_filter<?php if (version_compare(_PS_VERSION_, '1.6.0.12', '>=') && SCMS){ ?>,#select_filter<?php } ?>,#select_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter]]></param></call>
</beforeInit>
...
<column id="meta_title_width" width="40" type="ro" align="center" sort="str"><?php echo _l('META title length'); ?></column>
<column id="meta_description" width="200" type="ed" align="left" sort="str"><?php echo _l('META description'); ?></column>
<column id="meta_description_width" width="ro" type="ro" align="center" sort="str"><?php echo _l('META description length'); ?></column>
<column id="meta_keywords" width="120" type="ed" align="left" sort="str"><?php echo _l('META keywords'); ?></column>
<column id="meta_keywords_width" width="40" type="ro" align="center" sort="str"><?php echo _l('META keywords length'); ?></column>
<afterInit>
<call command="enableMultiselect"><param>1</param></call>
</afterInit>
</head>
<?php
    echo '<userdata name="uisettings">'.uisettings::getSetting('cms_prop_seo_grid').'</userdata>'."\n";
    echo $xml;
?>
</rows>
