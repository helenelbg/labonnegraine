<script type="text/javascript">
    ...
    cms_grid._uisettings_limited=true;
    ...
    cms_grid_tb.setIconset('awesome');
    var opts = [['cols123', 'obj', '<?php echo _l('Columns'); ?> 1 + 2 + 3', ''],
        ['cols12', 'obj', '<?php echo _l('Columns'); ?> 1 + 2', ''],
        ['cols23', 'obj', '<?php echo _l('Columns'); ?> 2 + 3', '']
    ];
    cms_grid_tb.addButtonSelect("layout", 100, "", opts, "fad fa-browser blue", "fad fa-browser blue",false,true);
    ...
    <?php
    sc_ext::readCustomCMSGridsConfigXML('gridnames');
    ?>
    var opts = new Array();
    ...
    cms_grid_tb.addButtonSelect("gridview", 100, "<?php echo _l('Light view'); ?>", opts, "fad fa-ruler-triangle", "fad fa-ruler-triangle",false,true);
    ...
    cms_grid_tb.addButtonSelect('lang',100,'<?php echo $clang; ?>',opts,'fad fa-flag blue','fad fa-flag blue',false,true);
    cms_grid_tb.setItemToolTip('lang','<?php echo _l('Select CMS language'); ?>');
    cms_grid_tb.setListOptionSelected('lang', '<?php echo $optlang; ?>');
    <?php
    }
    ?>
    cms_grid_tb.addButton("refresh", 100, "", "fa fa-sync green", "fa fa-sync green");
    cms_grid_tb.setItemToolTip('refresh','<?php echo _l('Refresh grid'); ?>');
    cms_grid_tb.addButton("add", 100, "", "fa fa-plus-circle green", "fa fa-plus-circle green");
    cms_grid_tb.setItemToolTip('add','<?php echo _l('Create new CMS page'); ?>');
    cms_grid_tb.addButton("delete", 100, "", "fa fa-minus-circle red", "fa fa-minus-circle red");
    cms_grid_tb.setItemToolTip('delete','<?php echo _l('This will permanently delete the selected CMS everywhere in the shop.'); ?>');
    cms_grid_tb.addButton("selectall", 100, "", "fa fa-bolt yellow", "fad fa-bolt grey");
    cms_grid_tb.setItemToolTip('selectall','<?php echo _l('Select all CMS'); ?>');
    cms_grid_tb.addButton("setposition", 100, "", "fa fa-list-ol green", "fa fa-list-ol grey");
    cms_grid_tb.setItemToolTip('setposition','<?php echo _l('Save CMS positions in the grid as category positions'); ?>');
    cms_grid_tb.addButton("exportcsv", 100, "", "fad fa-file-csv green", "fad fa-file-csv green");
    cms_grid_tb.setItemToolTip('exportcsv','<?php echo _l('Export grid to clipboard in CSV format for MSExcel with tab delimiter.'); ?>');
    getTbSettingsButton(cms_grid_tb, {'grideditor':1,'settings':0}, '', 100);
    cms_grid_tb.addButton("help", 1200, "", "fad fa-question-circle blue", "fad fa-question-circle blue");
    ...
    function gridToolBarOnClick(id){
        ...
        if (id=='help'){
            <?php echo "window.open('".getScExternalLink('support_cms')."');"; ?>
        }
        if (id=='filters_reset')
        {
        ...
        if (id=='exportcsv'){
            displayQuickExportWindow(cms_grid,1,null,null,true);
        }
        ...
        if (id=="delete")
        {
            if (confirm('<?php echo _l('This will permanently delete the selected CMS everywhere in the shop.', 1); ?>'))
            {
    ...
    function onEditCell(stage,rId,cInd,nValue,oValue){
        ...
        lastEditedCell=cInd;
        <?php
        sc_ext::readCustomCMSGridsConfigXML('onBeforeUpdate');
        ?>
        if (nValue!=oValue){
            ...
            if (cInd == idxName){
                ...
                <?php if (_s('CMS_SEO_META_TITLE_TO_URL')){ ?>
                    ...
                    { ?>
                    cms_grid.cells(rId,idxLinkRewrite).setValue(getLinkRewriteFromString(cms_grid.cells(rId,idxName).getValue().substr(0,<?php echo _s('CMS_LINK_REWRITE_SIZE'); ?>),SC_ID_LANG));
                    <?php } ?>
                <?php } ?>
            ...
            if (cInd == idxLinkRewrite){
                ...
                { ?>
                cms_grid.cells(rId,idxLinkRewrite).setValue(getLinkRewriteFromString(cms_grid.cells(rId,idxLinkRewrite).getValue().substr(0,<?php echo _s('CMS_LINK_REWRITE_SIZE'); ?>),SC_ID_LANG));
                <?php } ?>
            }
        ...
        <?php
        sc_ext::readCustomCMSGridsConfigXML('onEditCell');
        ?>
        if (nValue!=oValue){
            var id = rId;
    ...
    function onGridCmsContextButtonClick(itemId){
        ...
        if (itemId=="gopsbo"){
            ...
            wModifyCms.attachURL("<?php echo SC_PS_PATH_ADMIN_REL; ?>index.php?<?php echo version_compare(_PS_VERSION_, '1.5.0.0', '>=') ? 'controller=AdminCmsContent' : 'tab=AdminCMSContent'; ?>&updatecms&id_cms="+tabId+"&id_lang="+SC_ID_LANG+"&adminlang=1&token=<?php echo $sc_agent->getPSToken((version_compare(_PS_VERSION_, '1.5.0.0', '>=') ? 'AdminCmsContent' : 'AdminCMSContent')); ?>");
    ...
    function displayCms(callback)
    {
        if (cmsselection!=undefined && cmsselection!=null && cmsselection!="" && cmsselection!=0)
        {
            ...
            let loadUrl = "index.php?ajax=1&act=cms_page_get&tree_mode="+tree_mode+"&cmsfrom="+displayCmsFrom+"&idc="+cmsselection+"&view="+gridView+params_supp+"&id_lang="+SC_ID_LANG+"&"+new Date().getTime();
            ajaxPostCalling(cms_pagePanel, cms_grid, loadUrl,function(data)
            {
                cms_grid.parse(data);
                ...
                <?php sc_ext::readCustomCMSGridsConfigXML('afterGetRows'); ?>
