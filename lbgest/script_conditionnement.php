...
// catégories
$sql = 'SELECT cp.id_product, gn.id_categorie, gn.minimum, gn.optimum 
		FROM germination_normes gn
		LEFT JOIN ps_category_product cp ON cp.id_category = gn.id_categorie';
$requete_cat = $bdd->prepare($sql);
$requete_cat->execute() or die(print_r($requete_cat->errorInfo()));
$categories = $requete_cat->fetchAll(\PDO::FETCH_ASSOC|\PDO::FETCH_GROUP|\PDO::FETCH_UNIQUE);

$products = getProducts2(1);

// declinaisons
// ids 10512, 10513 = plants en précommande
$sql = 'SELECT *, sa.quantity as qte, pa.weight as poids
		...
		WHERE a.id_attribute NOT IN (10512,10513)
		AND a.id_attribute_group IN (6)
		AND al.id_lang = 1 
		ORDER BY pa.default_on DESC, a.position ASC';
$requete_decl = $bdd->prepare($sql);
$requete_decl->execute() or die(print_r($requete_decl->errorInfo()));
$declinaisons_all = $requete_decl->fetchAll(\PDO::FETCH_ASSOC);

$declinaisons_group = group_by($declinaisons_all, 'id_product');
...
foreach ($products as $product)
{
	...
	if(!isset($declinaisons_group[$id_product])){
		continue;
	}
	
	$declinaisons = $declinaisons_group[$id_product];
	
	foreach ($declinaisons as $dec_prod) {
		$name = trim($dec_prod['name']);
		if($name != 'Non traitée' && $name != 'BIO' && $name != 'plant'){
			$tab_dec[$id_product][$dec_prod['id_product_attribute']] = $name;
			$name = str_replace('Par ', '', $name);
			$exp = explode(' ', $name);
			$exp[0] = str_replace(',', '.', $exp[0]);
			if (isset($exp[1]) && strtolower($exp[1]) == 'kg' )
			{
				$tab_decqt[$id_product][$dec_prod['id_product_attribute']] = $exp[0] * 1000;
			}
			else
			{
				if ( isset($exp[0]) )
				{
					$tab_decqt[$id_product][$dec_prod['id_product_attribute']] = $exp[0];
				}
				else
				{
					$tab_decqt[$id_product][$dec_prod['id_product_attribute']] = $exp;
				}
			}
			if (isset($exp[1]) && strtolower($exp[1]) == 'graines' )
			{
			  $tab_unite[$id_product] = 'graines';
			}
			else {
			  $tab_unite[$id_product] = 'g';
			}

			if ( $dec_prod['default_on'] == 1 )
			{
			  $tab_stockd[$id_product] = $dec_prod['qte'];
			  $tab_stockdp[$id_product] = $dec_prod['poids'];
			}
			if(!isset($tab_stockf[$id_product]))
			{
				$tab_stockf[$id_product] = 0;
			}
			
			if (isset($exp[1]) && strtolower($exp[1]) != 'graines' )
			{
			  $tab_stockf[$id_product] += (int)$dec_prod['qte'] * (float)$dec_prod['poids'];
			}
			else
			{
			  $tab_stockf[$id_product] += (int)$dec_prod['qte'] * (float)$tab_decqt[$id_product][$dec_prod['id_product_attribute']];
			}
		}
	}
}
...
$sql = 'SELECT od.product_attribute_id, SUM(od.product_quantity) as qty FROM `ps_order_detail` AS od
LEFT JOIN `ps_orders` AS o ON od.id_order = o.id_order
WHERE o.invoice_date > "'.$moins.'" AND o.invoice_date < "'.$plus.'"
GROUP BY od.product_attribute_id';
$requeteG = $bdd->prepare($sql);
$requeteG->execute() or die(print_r($requeteG->errorInfo()));
$quantites = $requeteG->fetchAll(\PDO::FETCH_ASSOC|\PDO::FETCH_GROUP|\PDO::FETCH_UNIQUE);
...
foreach ($products as $product) {
	...
	if(isset($tab_dec[$product['id_product']])) {
		foreach ($tab_dec[$product['id_product']] as $key => $value) {
			...
			$nb_sachet_vendu = 0;
			if(isset($quantites[$key])){
				$nb_sachet_vendu = $quantites[$key]['qty'];
			}
			...
			if ( isset($tab_decqt[$product['id_product']][$key])){
				...
				if ( $nb_stock > 0) {
					...
					if ( !isset($tableau_final_germ[$product['id_product']]) )
					{
						...
						if(isset($categories[$product['id_product']])){
							$cat = $categories[$product['id_product']];
						}
