<?php
    class Commande
    {
        ...
        public static function getProductsByOrderGroup($orders)
        {
            ...
            {      
                $req = new DbQuery();
                $req->select('lgcpo.location, od.id_order, od.product_id, od.product_attribute_id, od.product_name, od.product_quantity, od.product_quantity_refunded, product_ean13, product_reference, pa.default_on, pl.name');
                $req->from('order_detail', 'od');
                $req->leftJoin('product_attribute', 'pa', 'od.product_attribute_id = pa.id_product_attribute');
                $req->leftJoin('product_lang', 'pl', 'od.product_id = pl.id_product');
                ...
                $req->where('pl.id_lang = "1"');
                $req->where('od.product_id <> "3128" AND od.product_id <> "1850" AND od.product_id <> "1851" AND od.product_id <> "2638" AND od.product_id <> "1849"');//Carte cadeau et box
                $req->orderBy('lgcpo.`ordre` ASC, product_reference ASC');
        ...
        {
            $req_c = 'SELECT id_controle, id_order FROM ps_LogiGraine_controle WHERE id_operateur = "'.$id_operateur.'" AND valide = 0 LIMIT 0,1;';
