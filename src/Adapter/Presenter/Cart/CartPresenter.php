...
class CartPresenter implements PresenterInterface
{
    ...
    public function present($cart, $shouldSeparateGifts = false)
    {
        ...
        $auxTotalCartAmount = $cart->getOrderTotalDetail($this->includeTaxes(), Cart::ONLY_PRODUCTS);
        if ( isset($auxTotalCartAmount['detail']) && count($auxTotalCartAmount['detail']) > 1 )
        {
            $totalCartAmount = -1;
        }
        else 
        ...
        {
            $cptCA = 1;
            foreach ($auxTotalCartAmount['detail'] as $detailCA)
            {
                $subtotals['products'.$cptCA] = [
                    'type' => 'products',
                    'label' => $this->translator->trans('Subtotal', [], 'Shop.Theme.Checkout').' Commande n°'.$cptCA,
                    'amount' => $detailCA,
                    //'value' => $this->getShippingDisplayValue($cart, $detailCA),
                    'value' => $this->priceFormatter->format($detailCA),
                ];
                $cptCA++;
            }
        }
        ...
        if (!$cart->isVirtualCart()) {
            $auxShippingCost = $cart->getTotalShippingCostDetail(null, $this->includeTaxes());
            if ( count($auxShippingCost['detail']) > 1 )
            {
                $shippingCost = -1;
            }
            else 
        ...
        {
            $cptSC = 1;
            foreach ($auxShippingCost['detail'] as $detailSC)
            {
                if ( $this->includeTaxes() )
                {
                    $shippingCost = $detailSC['with_tax'];
                }
                else 
                {
                    $shippingCost = $detailSC['without_tax'];
                }
                if ( $shippingCost == 0 )
                {
                    $shippingCost = 'Offerte';
                }
                $subtotals['shipping'.$cptSC] = [
                    'type' => 'shipping',
                    'label' => $this->translator->trans('Shipping', [], 'Shop.Theme.Checkout').' Commande n°'.$cptSC,
                    'amount' => $shippingCost,
        ...
        $products_count_detail = array();
        foreach ($products as $productCount) 
        {
            if ( !isset($products_count_detail[$productCount['warehouse_en_cours']]) )
            {
                $products_count_detail[$productCount['warehouse_en_cours']] = 0;
            }
            $products_count_detail[$productCount['warehouse_en_cours']] += $productCount['quantity'];
            
        }

        $cpt_pcd = 1;
        $summary_string_detail = array();
        foreach($products_count_detail as $whEC => $pcd)
        {
            $summary_string_detail[$cpt_pcd] = $products_count_detail[$whEC] === 1 ?
            $this->translator->trans('1 item', [], 'Shop.Theme.Checkout') :
            $this->translator->trans('%count% items', ['%count%' => $products_count_detail[$whEC]], 'Shop.Theme.Checkout');
        ...
        $result = [
            ...
            'summary_string_detail' => $summary_string_detail,
            'labels' => $labels,
    ...
    {
        //error_log('$shippingCost :::::::::::::::::: '.$shippingCost);
        ...
        if ($shippingCost != 0) {
            ...
            if ( is_array($deliveryOptionList) )
            {
    ...
    private function getTemplateVarVouchers(Cart $cart)
    {
        ...
        foreach ($cartVouchers as $cartVoucher) {
            ...
            if ( isset($cartVoucher['reduction_product']) && $cartVoucher['reduction_product'] > 0 )
            {
                $vouchers[$cartVoucher['id_cart_rule']]['reduction_product'] = $cartVoucher['reduction_product'];
            }
            else
            {
                $vouchers[$cartVoucher['id_cart_rule']]['reduction_product'] = 0;
            }
    ...
    private function cartVoucherHasFreeShipping(array $cartVoucher): bool
    {
        error_log('$cartVoucher[free_shipping] : ' .$cartVoucher['free_shipping']);
        if ( $cartVoucher['free_shipping'] == 1 )
        {
            return true;
        }
        return false;
    ...
    private function cartVoucherHasFreeShippingOnly(array $cartVoucher): bool
    {
        return !$this->cartVoucherHasPercentReduction($cartVoucher)
            ...
            && $this->cartVoucherHasFreeShipping ($cartVoucher);
    }
