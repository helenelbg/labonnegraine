...
class CartRow
{
    ...
    protected function getProductPrice(CartCore $cart, $rowData)
    {
        ...
        //echo '############ > '.$productPrices['price_tax_included']['value'].'<br />';
    ...
    /*protected function applyRound($cart = null)
    {
        // ROUNDING MODE
        $this->finalUnitPrice = clone $this->initialUnitPrice;
        ...
        if ( $cart )
        {
            $sql = 'SELECT SUM(reduction_amount) AS reduc
                                            ...
                                            WHERE `id_cart` = '.(int)$cart->id.' AND (code LIKE "CC_%" OR crl.name = "La carte cadeau") ORDER by cr.priority ASC';
            $result_CC_TTC = $this->databaseAdapter->getValue($sql, _PS_USE_SQL_SLAVE_);
            if ($result_CC_TTC > 0)
            {
                $result_CC_HT = $result_CC_TTC / (($this->initialUnitPrice->getTaxIncluded())/($this->initialUnitPrice->getTaxExcluded()));
                //echo $result_CC_TTC.'<br />';
                //echo $result_CC_HT.'<br />';
                //echo $productPrices['price_tax_included']['value'].'<br />';
                //echo $productPrices['price_tax_excluded']['value'].'<br /><hr>';
                //$productPrices['price_tax_included']['value'] += $result_CC_TTC;
                //$productPrices['price_tax_excluded']['value'] += $result_CC_HT;
            }
        }
        ...
        switch ($this->roundType) {
            case self::ROUND_MODE_TOTAL:
                // do not round the line
                $this->finalTotalPrice = new AmountImmutable(
                    ($this->initialUnitPrice->getTaxIncluded() * $quantity)+$result_CC_TTC,
                    ($this->initialUnitPrice->getTaxExcluded() * $quantity)+$result_CC_HT
                );
            ...
            case self::ROUND_MODE_LINE:
                // round line result
                $this->finalTotalPrice = new AmountImmutable(
                    $tools->round(($this->initialUnitPrice->getTaxIncluded() * $quantity)+$result_CC_TTC, $this->precision),
                    $tools->round(($this->initialUnitPrice->getTaxExcluded() * $quantity)+$result_CC_HT, $this->precision)
                );
