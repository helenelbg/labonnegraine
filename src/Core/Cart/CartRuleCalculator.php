...
class CartRuleCalculator
{
    ...
    protected function applyCartRule(CartRuleData $cartRuleData, $withFreeShipping = true)
    {
        ...
        if ((float) $cartRule->reduction_amount > 0) {
            ...
            //error_log('_________________packageSimple_________________________');
            //error_log(print_r($cart->packageSimple, true));         
            if ( isset($cart->packageSimple) && !empty($cart->packageSimple) )
            {
                $packageSimple = $cart->packageSimple; 
                $encoursSimple = array();
                foreach ($this->cartRows as $cartRow) {
                    $product = $cartRow->getRowData();
                    $encoursSimple[] = $product['id_product'];
                } 
                //error_log('_________________encoursSimple_________________________');
                //error_log(print_r($encoursSimple, true)); 

                $packageATraiter = 0;
                $arrayATraiter = array();   
                foreach ($packageSimple as $idPackage => $simple) {    
                        foreach($simple as $prodPackageEC)
                        {
                            $key = array_search($prodPackageEC['id_product'], $encoursSimple);
                            if ( $key !== false )
                            {
                                $packageATraiter = $idPackage;
                                $arrayATraiter[] = $idPackage;
                            }
                        }
                }
                $arrayATraiter = array_unique($arrayATraiter);
                $montantADeduire = 0;
                //error_log('packageATraiter : '.$packageATraiter);   
                //error_log(print_r($arrayATraiter, true));    
                if ( count($arrayATraiter) == 1 )
                {
                    foreach ($packageSimple as $idPackage => $simple) {      
                        if ( $idPackage < $packageATraiter )
                        {
                            foreach($simple as $prodPackageEC)
                            {
                                $montantADeduire += $prodPackageEC['price'];
                            }
                        }
                    }
                    //error_log('montantADeduire : '.$montantADeduire);   
                    $cartRule->reduction_amount = $cartRule->reduction_amount - $montantADeduire;
                    if ( $cartRule->reduction_amount < 0 )
                    {
                        $cartRule->reduction_amount = 0;
                    }
                    //error_log('$cartRule->reduction_amount : '.$cartRule->reduction_amount); 
                }
                }
            ...
            if ($cartRule->reduction_product == -2) 
            {
                ...
                if (is_array($selected_products)) {
                    foreach ($this->cartRows as $cartRow) {
                        ...
                        if ((in_array($product['id_product'] . '-' . $product['id_product_attribute'], $selected_products)
                                || in_array($product['id_product'] . '-0', $selected_products))
                            && (($cartRule->reduction_exclude_special && !$product['reduction_applies']) || !$cartRule->reduction_exclude_special)) {
                            $price = $product['price'];
