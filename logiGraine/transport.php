                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                if(data.startsWith('Erreur')){
                                    ...
                                    {
                                        window.open(exp[1]);
                                        console.log('window.open id_order : '+id_order);
                                        window.open('https://www.labonnegraine.com/admin123/index.php/sell/orders/'+id_order+'/generate-invoice-pdf');
                                    }
                                    $.ajax({
                                        method: "POST",
        ...
        <?php 
            ...
            if ( isset($idCommandeEC) && !empty($idCommandeEC) )
            {
                ...
                if ( $carrierEC->id_reference == 348 || $carrierEC->id_reference == 142 || $carrierEC->id_reference == 189 || $carrierEC->id_reference == 190 || $carrierEC->id_reference == 191 || $carrierEC->id_reference == 192 || $carrierEC->id_reference == 112 )
                {
                    ...
                    echo '<br /><br /><input type="button" value="Imprimer une étiquette supplémentaire" onclick="nouvelle_etiquette('.$idCommandeEC.');" />';
                }
            }
            elseif ( isset($_POST['codeCaisse']) && !empty($_POST['codeCaisse']) )
            {
                $derniereCommande = Caisse::getDerniereCommandeByCode($_POST['codeCaisse']);
                $explode_dc = explode('#', $derniereCommande);
                echo "Dernière commande avec cette caisse : <b>".$explode_dc[0].'</b> ('.substr($explode_dc[1], 8, 2).'/'.substr($explode_dc[1], 5, 2).'/'.substr($explode_dc[1], 0, 4).' '.substr($explode_dc[1], 11, 8).')';
            } 
        ?>
      </form>
    </div>
...
<script type="text/javascript">
    ...
    function nouvelle_etiquette(id_order)
    {
        var pAjax = new Object();
        //var token = $('#token_etiquetage').val();

        pAjax.type = 'POST';
        pAjax.url = '/modules/controlecommande/generateColissimo.php?new=ok&poste='+localStorage.getItem('poste'); 
        pAjax.data_type = 'html';
        pAjax.data = 'id_order='+id_order; 
