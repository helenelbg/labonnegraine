<?php
    ...
    foreach ( Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($req) as $rangee )
    {
        ...
        $req_cmd = 'SELECT SUM(product_quantity) AS nb FROM ps_orders o LEFT JOIN ps_order_detail od ON o.id_order = od.id_order WHERE current_state IN (2,3) AND product_ean13 = "'.$rangee['ean'].'";';
        $rangee_cmd = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($req_cmd);

        if ( ($rangee['quantity'] - $rangee_cmd[0]['nb']) < 0 )
        {
