<?php
    class Commande
    {
        ...
        public static function getCommandesLettresVertesByPda($id_pda = 0)
        {
            if ( $id_pda > 0 )
            {
                $req = new DbQuery();
                ...
                $req->from('LogiGraine_pda_order', 'lgrm');
                $req->leftJoin('LogiGraine_controle', 'lgc', 'lgrm.`id_order` = lgc.`id_order`');
                $req->leftJoin('orders', 'o', 'lgc.`id_order` = o.`id_order`');
                $req->leftJoin('carrier', 'c', 'o.`id_carrier` = c.`id_carrier`');
                $req->where('lgrm.id_pda = "'.$id_pda.'"');
                $req->where('lgc.valide = 0');
                $req->where('c.id_reference = 342');
                $req->orderBy('lgrm.`id_pda_order` ASC');
                //echo $req->build();
                $resu = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($req);
        ...
        public static function getCommandesLettresVertesByPdaForEtiquettes($id_pda = 0)
        {
            if ( $id_pda > 0 )
            {
              $req_o = 'SELECT id_operateur FROM ps_LogiGraine_pda_operateur WHERE id_pda = "'.$id_pda.'";';
              ...
              $req->leftJoin('orders', 'o', 'lgc.`id_order` = o.`id_order`');
              $req->leftJoin('carrier', 'c', 'o.`id_carrier` = c.`id_carrier`');
              $req->where('lgc.id_operateur = "'.$operateur.'"');
              $req->where('lgc.valide = 1');
              $req->where('lgc.transport = 0');
              $req->where('c.id_reference = 342');
              $req->orderBy('lgc.`id_controle` ASC');
                //echo $req->build();
                $resu = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($req);
        ...
        public static function getProductsByOrder($id_order = 0)
        {
            ...
            {
                $req = new DbQuery();
                $req->select('od.product_id, od.product_attribute_id, od.product_name, od.product_quantity, od.product_quantity_refunded, product_ean13, product_reference, pa.default_on, pl.name, p.product_type');
                ...
                $req->leftJoin('product', 'p', 'od.product_id = p.id_product');
                ...
                foreach($resu as $rangee)
                {
                    ...
                    else 
                    {
                        ...
                        $req_pack->select('pa.id_product_item, pa.id_product_attribute_item, pl.name, pa.quantity, p.ean13 as ean13_p, pat.ean13, p.reference as reference_p, pat.reference, pat.default_on');
                        $req_pack->from('pack', 'pa');
                        $req_pack->leftJoin('product_lang', 'pl', 'pa.id_product_item = pl.id_product');
                        $req_pack->leftJoin('product', 'p', 'pa.id_product_item = p.id_product');
                        $req_pack->leftJoin('product_attribute', 'pat', 'pa.id_product_attribute_item = pat.id_product_attribute');
                        $req_pack->where('pa.id_product_pack = "'.$rangee['product_id'].'"');
                        $req_pack->where('pl.id_lang = "1"');

                        $resu_pack = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($req_pack);
                        foreach($resu_pack as $rangee_pack)
                        {
                            $rangee_pack['quantity_final'] = ($rangee_pack['quantity']*$rangee['product_quantity']) - ($rangee_pack['quantity']*$rangee['product_quantity_refunded']);
                            if ( !empty($rangee_pack['ean13']) )
                            {
                                $ean13EC = $rangee_pack['ean13'];
                            }
                            else
                            {                                
                                $ean13EC = $rangee_pack['ean13_p'];
                            }
                            if ( !empty($rangee_pack['reference']) )
                            {
                                $referenceEC = $rangee_pack['reference'];
                            }
                            else
                            {                                
                                $referenceEC = $rangee_pack['reference_p'];
                            }
                            ...
                            WHERE pa.id_product_attribute = ' . (int)$rangee_pack['id_product_attribute_item'] . '
                            AND al.id_lang = ' . (int)Context::getContext()->language->id . '
                            AND agl.id_lang = ' . (int)Context::getContext()->language->id;
                    
                            $attributes = Db::getInstance()->executeS($sql_c);
                            $attributes_txt = '';
                            foreach ($attributes as $attribute) {
                                if ( !empty($attributes_txt) )
                                {
                                    $attributes_txt .= ', ';
                                }
                                $attributes_txt .= $attribute['attribute_name'];
                            }

                            $packEC = array('product_id' => $rangee_pack['id_product_item'], 'product_attribute_id' => $rangee_pack['id_product_attribute_item'], 'product_name' => $rangee_pack['name'], 'product_quantity' => ($rangee_pack['quantity']*$rangee['product_quantity']), 'product_quantity_refunded' => ($rangee_pack['quantity']*$rangee['product_quantity_refunded']), 'product_ean13' => $ean13EC, 'product_reference' => $referenceEC, 'default_on' => $rangee_pack['default_on'], 'name' => $rangee_pack['name'], 'quantity_final' => $rangee_pack['quantity_final'], 'product_name_1' => $rangee_pack['name'], 'product_name_2' => $attributes_txt);
                            $resultat[] = $packEC;
                        }
                    }

                    /*if ( $rangee['product_id'] == 2154 )
                    {
                        // Cas particulier des godet avec barquette (on rajoute la barquette pour le picking)
                        $barq = array('product_id' => 999999, 'product_attribute_id' => 0, 'product_name' => 'Barquette', 'product_quantity' => $rangee['product_quantity'], 'product_quantity_refunded' => $rangee['product_quantity_refunded'], 'product_ean13' => '1234567891231', 'product_reference' => $rangee['product_reference'], 'default_on' => $rangee['default_on'], 'name' => 'Barquette', 'quantity_final' => $rangee['quantity_final'], 'product_name_1' => 'Barquette', 'product_name_2' => 'Barquette');
                        $resultat[] = $barq;
                    }*/
                }
        ...
        public static function getProductsByOrderGroup($orders)
        {
            if ( $orders > 0 )
            {
                $req = new DbQuery();
                $req->select('if(lgcpo.ordre IS NULL, 99999, lgcpo.ordre) AS ordreEC, lgcpo.location, od.id_order, od.product_id, od.product_attribute_id, od.product_name, od.product_quantity, od.product_quantity_refunded, product_ean13, product_reference, pa.default_on, pl.name, p.product_type');
                $req->from('order_detail', 'od');
                $req->leftJoin('product_attribute', 'pa', 'od.product_attribute_id = pa.id_product_attribute');
                $req->leftJoin('product_lang', 'pl', 'od.product_id = pl.id_product');
                $req->leftJoin('product', 'p', 'od.product_id = p.id_product');
                $req->leftJoin('LogiGraine_controle_produit_ordre', 'lgcpo', 'od.id_order = lgcpo.id_order AND od.product_ean13 = lgcpo.ean');
                $req->where('od.id_order IN ('.str_replace('_', ',', $orders).')');
                $req->where('pl.id_lang = "1"');
                ...
                $req->orderBy('ordreEC ASC, product_reference ASC');
                //echo $req->build();
                $resu = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($req);
                ...
                foreach($resu as $rangee)
                {
                    ...
                    {
                        $req_pack = new DbQuery();
                        $req_pack->select('if(lgcpo.ordre IS NULL, 99999, lgcpo.ordre) AS ordreEC, lgcpo.location, pa.id_product_item, pa.id_product_attribute_item, pl.name, pa.quantity, p.ean13 as ean13_p, pat.ean13, p.reference as reference_p, pat.reference, pat.default_on');
                        $req_pack->from('pack', 'pa');
                        $req_pack->leftJoin('product_lang', 'pl', 'pa.id_product_item = pl.id_product');
                        $req_pack->leftJoin('product', 'p', 'pa.id_product_item = p.id_product');
                        $req_pack->leftJoin('product_attribute', 'pat', 'pa.id_product_attribute_item = pat.id_product_attribute');
                        $req_pack->leftJoin('LogiGraine_controle_produit_ordre', 'lgcpo', 'pat.ean13 = lgcpo.ean');
                        $req_pack->where('pa.id_product_pack = "'.$rangee['product_id'].'"');
                        $req_pack->where('lgcpo.id_order = "'.$rangee['id_order'].'"'); 
                        $req_pack->where('pl.id_lang = "1"');                        
                        ...
                        foreach($resu_pack as $rangee_pack)
                        {
                            ...
                            $packEC = array('ordreEC' => $rangee_pack['ordreEC'], 'location' => $rangee_pack['location'], 'id_order' => $rangee['id_order'], 'product_id' => $rangee_pack['id_product_item'], 'product_attribute_id' => $rangee_pack['id_product_attribute_item'], 'product_name' => $rangee_pack['name'], 'product_quantity' => ($rangee_pack['quantity']*$rangee['product_quantity']), 'product_quantity_refunded' => ($rangee_pack['quantity']*$rangee['product_quantity_refunded']), 'product_ean13' => $ean13EC, 'product_reference' => $referenceEC, 'default_on' => $rangee_pack['default_on'], 'name' => $rangee_pack['name'], 'quantity_final' => $rangee_pack['quantity_final'], 'product_name_1' => $rangee_pack['name'], 'product_name_2' => $attributes_txt);
                            $resultat[] = $packEC;
                        }
                    }
                    ...
                    {
                        // Cas particulier des godet avec barquette (on rajoute la barquette pour le picking)
                        $barq = array('ordreEC' => $rangee['ordreEC'], 'location' => $rangee['location'], 'id_order' => $rangee['id_order'], 'product_id' => 999999, 'product_attribute_id' => 0, 'product_name' => 'Barquette', 'product_quantity' => $rangee['product_quantity'], 'product_quantity_refunded' => $rangee['product_quantity_refunded'], 'product_ean13' => '1234567891231', 'product_reference' => $rangee['product_reference'], 'default_on' => $rangee['default_on'], 'name' => 'Barquette', 'quantity_final' => $rangee['quantity_final'], 'product_name_1' => 'Barquette', 'product_name_2' => '-');
                        $resultat[] = $barq;
                    }*/
                }
                return $resultat;
        ...
        public static function getProductsByOrderGroup2($orders)
        {
            if ( $orders > 0 )
            {
                $req = new DbQuery();
                $req->select('if(lgcpo.ordre IS NULL, 99999, lgcpo.ordre) AS ordreEC, lgcpo.location, od.id_order, od.product_id, od.product_attribute_id, od.product_name, od.product_quantity, od.product_quantity_refunded, product_ean13, product_reference, pa.default_on, pl.name');
                $req->from('order_detail', 'od');
                $req->leftJoin('product_attribute', 'pa', 'od.product_attribute_id = pa.id_product_attribute');
                $req->leftJoin('product_lang', 'pl', 'od.product_id = pl.id_product');
                $req->leftJoin('LogiGraine_controle_produit_ordre', 'lgcpo', 'od.id_order = lgcpo.id_order AND od.product_ean13 = lgcpo.ean');
        ...
        public static function getATraiter()
        {
            $nb_session = 0;
            $nb_cmd = 0;

            $req = 'SELECT * FROM ps_LogiGraine_groupes_commandes WHERE statut = "disponible";';
            $resu = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($req);

            foreach($resu as $rangee)
            {
                $nb_session++;
                $nb_cmd += count(explode('_', $rangee['id_order']));
            }

            return $nb_session.'_'.$nb_cmd;
        }
            ...
            if ( isset($commandes) && is_array($commandes) )
            {
                ...
                if ( !empty($rowsins) )
                {
                    ...
                    $req_comp = 'INSERT INTO ps_LogiGraine_groupes_commandes
                                    (date_creation, statut, date_prise, id_order)
                    SELECT date_creation, statut, date_prise, id_order FROM temp_groupes_commandes;';
        ...
        public static function getSession($id_operateur)
        {
            $req_c = 'SELECT c.id_controle, c.id_order FROM ps_LogiGraine_controle c LEFT JOIN ps_orders o ON c.id_order = o.id_order LEFT JOIN ps_carrier ca ON o.id_carrier = ca.id_carrier WHERE ca.id_reference <> 342 AND c.id_operateur = "'.$id_operateur.'" AND c.valide = 0 LIMIT 0,1;';
            $resu_c = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($req_c);

            $retour = array();
/*if ( $id_operateur == 2 )
{
    error_log(print_r($resu_c, true));
}*/
            if ( isset($resu_c[0]['id_controle']) )
            {
                $req_g = 'SELECT id_order FROM ps_LogiGraine_groupes_commandes WHERE (id_order LIKE "%_'.$resu_c[0]['id_order'].'_%" OR id_order LIKE "'.$resu_c[0]['id_order'].'_%" OR id_order LIKE "%_'.$resu_c[0]['id_order'].'" OR id_order = "'.$resu_c[0]['id_order'].'");';
                /*if ( $id_operateur == 2 )
                {
                    error_log($req_g);
                }*/
