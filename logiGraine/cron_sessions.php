...
{
    $req = 'SELECT distinct(o.id_order) FROM ps_orders o LEFT JOIN ps_carrier c ON o.id_carrier = c.id_carrier LEFT JOIN ps_order_detail od ON o.id_order = od.id_order WHERE o.id_order > 228688 AND o.current_state IN (2,47) AND c.id_reference <> 342 AND (od.id_warehouse = 0 OR od.id_warehouse <= '.date('W').') AND o.id_order NOT IN ('.implode(',', $cmd_rosiers).');';
}
else 
{
    $req = 'SELECT distinct(o.id_order) FROM ps_orders o LEFT JOIN ps_carrier c ON o.id_carrier = c.id_carrier LEFT JOIN ps_order_detail od ON o.id_order = od.id_order WHERE o.id_order > 228688 AND  o.current_state IN (2,47) AND c.id_reference <> 342 AND (od.id_warehouse = 0 OR od.id_warehouse <= '.date('W').');';
}
