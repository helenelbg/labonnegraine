...
$(document).ready(() => {
  ...
  $(document).on('click', '.js-aw-show-orders', function() {
	const cart_rule_id = parseInt($('.column-id_cart_rule',$(this).closest('tr')).html());
	const name = $('.column-name',$(this).closest('tr')).html();
	...
	$.ajax({
		url: 'index.php?controller=AdminCartRules',
		method: 'POST',
		data: {
			ajax: true,
			action: 'showOrders',
		...
		success: function (response) {
			...
			if (res.success) {
				const html = '<div class="modal-aw-orders-title">'+cart_rule_id + ' ' +name+'</div>' + res.message;
				$('.modal-section-aw-orders').html(html);
				$('.modal-box-aw-orders').show();
			} else {
