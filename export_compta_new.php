<?php
    ...
    foreach($orders as $order)
    {
        ...
        //Taxes à 10, 20 et 5.5
        $tvatwenty = 0; $tvatwentyamount = 0;
        $tvafive = 0; $tvafiveamount = 0;
        ...
        foreach($prod_array as $id)
        {
            ...
            elseif($detail_group[0]['id_tax_rules_group'] == "4") 
            {
                $ratefive = ($detail_taxec[0]['rate']/100)+1;
                $tvatenamount -= $id['total_price_tax_incl'];
                $tvafiveamount += $id['total_price_tax_incl'];
            }
            else
            {
                $tvatenamount -= $id['total_price_tax_incl'];
        ...
        $tvaten = round($tvaten,6);
        $tvatenamount = round($tvatenamount,6);
        $tvatwenty = round($tvatwenty,6);
        $tvatwentyamount = round($tvatwentyamount,6);
        $tvafive = round($tvafive,6);
        $tvafiveamount = round($tvafiveamount,6);
        ...
        if ( $tvafiveamount > 0 )
        {
            $tvafive = round($tvafiveamount - ($tvafiveamount / $ratefive),2);
        }

        $tvatenamount -= $tvaten;
        $tvatwentyamount -=  $tvatwenty;
        $tvafiveamount -=  $tvafive;
        ...
        if($cartrules = Db::getInstance()->ExecuteS($query_cartrule)) 
        {
            foreach($cartrules as $onerule) 
            {
                ...
                else 
                {
                    ...
                    $reduc_55 += round(($onerule['value']/$ratefive), 2);
                    $montant_reduc_55 += $onerule['value'];
        ...
        if(floatval($order['total_discounts']) != 0 && $tvatenamount > 0)
        {
            ...
            else
            {
                if ( $test_export == 0 )
                {
                    ...
                    $reduc_55 -= $reduc_10;
                    $montant_reduc_55 -= $montant_reduc_10;
                }
        ...
        if ( $tvatenamount > 0 )
        {
            ...
            elseif ( $test_export == 1 )
            {
                $result_file .= round($tvafiveamount,2)+$tvafive+round($tvatenamount,2)+$tvaten+round($tvatwentyamount,2)+$tvatwenty."\n"; //$order['total_products']
                $colonne2 += round($tvafiveamount,2)+$tvafive+round($tvatenamount,2)+$tvaten+round($tvatwentyamount,2)+$tvatwenty;
            }
        }

        // TVA 5.5%
        if($tvafive != 0 && $test_export == 0) 
        {
            $result_file .= $debut_commun;

            $result_file .= ";70710000;";
            ...
            if(floatval($order['total_discounts']) != 0)
            {
                ...
                {
                    $result_file .= round($reduc_55,2);
                    $colonne1 += round($reduc_55,2);
                }
            }

            $result_file .= ";";
            $result_file .= round($tvafiveamount,2)."\n";
            $colonne2 += round($tvafiveamount,2);
        }
        ...
        if($reduc_55)
        {
            $tva_reduc_3 = $montant_reduc_55 - $reduc_55;
        }
        ...
        if(floatval($total_tva) != 0 && $test_export == 0)
        {
            if ( in_array($id_zone, $tabue) )
            {
                ...
                if(($tva_reduc_1+$tva_reduc_2+$tva_reduc_3) != 0)
                {
                    $result_file .= round($tva_reduc_1+$tva_reduc_2+$tva_reduc_3,2).";";
                    $colonne1 += round($tva_reduc_1+$tva_reduc_2+$tva_reduc_3,2);
                }
                else
                {
                    $result_file .= ';';
            ...
            else
            {
                if ( floatval($total_tva_transport) != 0 )
                {
                    ...
                    {
                        $result_file .= $debut_commun;
                        $result_file .= ";44571100;";
                        
                        if($tva_reduc_3 != 0)
                        {
                            $result_file .= round($tva_reduc_3,2).";";
                            $colonne1 += round($tva_reduc_3,2);
                        }
                        else
                        {
                            $result_file .= ';';
                        }
                        $result_file .= round($tvafive,2)."\n";
                        $colonne2 += round($tvafive,2);
                    }
                    $result_file .= $debut_commun;
    ...
    foreach($orders as $order)
    {
        ...
        foreach($prod_array as $id)
        {
            ...
            elseif($detail_group[0]['id_tax_rules_group'] == "4") 
            {
                $ratefive = ($detail_taxec[0]['rate']/100)+1;
                $tvatenamount -= $id['amount_tax_incl'];
                $tvafiveamount += $id['amount_tax_incl'];
            }
            else
            {
                $tvatenamount -= $id['amount_tax_incl'];
